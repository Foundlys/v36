...
const field=(root,label)=>root.all().find(el=>el.tag==='label'&&el.children.some(child=>child.tag==='span'&&child.textContent===label)).children.find(child=>child.tag==='select');
...
(async()=>{
 ...
 denied=true;await button.fire('click');assert.equal(find(box,'h4'),undefined);assert.equal(find(box,'output').textContent,'Rungegevens konden niet worden opgehaald.');denied=false;
