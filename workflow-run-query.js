...
function queryRuns(core,ctx,actor,query={}){
  ...
  const statuses=String(query.status||'').split(',').filter(Boolean).map(s=>s.toUpperCase());if(statuses.some(status=>!['RUNNING','PLANNED','SUCCEEDED','ERROR','BLOCKED','AWAITING_APPROVAL','WAITING_TIME','WAITING_RETRY','DEAD_LETTER','RECOVERY_READY'].includes(status)))fail('automation_query_status_invalid','Onbekende uitvoerstatus');
