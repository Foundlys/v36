...
(async()=>{try{
 ...
 const amountDate={...scenario,adjustments:[{opportunity_id:row.id,expected_revision:1,value_cents:12345,expected_close_date:'2026-10-01'}]};let shifted=await call(route,'POST',{filters,scenario:amountDate});assert.equal(shifted.status,200,JSON.stringify(shifted.body));assert.equal(shifted.body.scenario.groups[0].scenario_projected_cents,0);assert.equal(shifted.body.scenario.items[0].scenario_value_cents,12345);assert.equal(shifted.body.scenario.items[0].included_in_period,false);assert.equal((await call(route,'POST',{filters,scenario:{...amountDate,adjustments:[{...amountDate.adjustments[0],expected_close_date:'2026-02-30'}]}})).status,422);assert.deepEqual((await call('/api/sales/opportunities/'+row.id)).body.record,row);
