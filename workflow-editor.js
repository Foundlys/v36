...
(function(root){
  ...
  function create({document,spec,request,onSaved,draft:initial=null,zeroRequest=null,requestContext,isActive=()=>true,onDraftRecovered=()=>{}}){
    ...
    const storageKey=realm?'foundly.workflow.editor.drafts.v1:'+JSON.stringify([realm.tenant_id,realm.dealer_id,realm.actor_id]):null,prior=[];let storageReady=true;
    ...
    function readPending(){if(!storageKey)throw storageError();const raw=root.sessionStorage.getItem(storageKey),rows=raw===null?[]:JSON.parse(raw);if(!Array.isArray(rows)||rows.length>100||!rows.every(validMetadata)||new Set(rows.map(row=>row.request_id)).size!==rows.length)throw storageError();return rows;}
    ...
    function remember(meta){
      ...
      if(prior.some(row=>!row.done))throw Object.assign(Error('workflow_draft_recovery_required'),{draftRecoveryRequired:true});
      try{if(!storageReady||!validMetadata(meta))throw storageError();const rows=readPending(),existing=rows.find(row=>row.request_id===meta.request_id);if(existing){if(!same(existing,meta))throw storageError();return;}if(rows.length>=100)throw storageError();storePending([...rows,meta]);}catch{storageReady=false;throw storageError();}
    }
    function forget(meta){
      if(!active())throw Object.assign(Error('workflow_draft_session_inactive'),{code:'workflow_draft_session_inactive'});
      try{const rows=readPending(),existing=rows.find(row=>row.request_id===meta.request_id);if(existing&&!same(existing,meta))throw storageError();storePending(rows.filter(row=>row.request_id!==meta.request_id));}catch{storageReady=false;throw storageError();}
    }
    ...
    function feedback(error,key='error'){
      if(error.draftStorage){generatorMessage('storage_required');return;}if(error.draftRecoveryRequired){message('recovery_pending');return;}
    ...
    if(recoveryEnabled)try{for(const entry of readPending())showRecovery(entry);}catch{storageReady=false;generatorMessage('storage_required');}
