...
(function(root){
  ...
  function create({document,spec,request,onSaved,draft:initial=null,zeroRequest=null,requestContext,isActive=()=>true,onDraftRecovered=()=>{}}){
    ...
    const recoveryEnabled=Boolean(requestContext),realm=record(requestContext)&&['tenant_id','dealer_id','actor_id'].every(key=>typeof requestContext[key]==='string'&&requestContext[key].length>0&&requestContext[key].length<=200)?requestContext:null;
    const storageKey=kind=>realm?'foundly.workflow.editor.'+(kind==='publication'?'publications':'drafts')+'.v1:'+JSON.stringify([realm.tenant_id,realm.dealer_id,realm.actor_id]):null,prior=[];let storageReady=true;
    const id=value=>typeof value==='string'&&/^[A-Za-z0-9][A-Za-z0-9_-]{0,99}$/.test(value),validMetadata=value=>record(value)&&Object.keys(value).length===4&&id(value.draft_id)&&id(value.request_id)&&Number.isSafeInteger(value.expected_revision)&&value.expected_revision>=0&&hash(value.request_fingerprint);
    const storageError=()=>Object.assign(Error('workflow_draft_storage_required'),{draftStorage:true});
    const validPublication=value=>record(value)&&Object.keys(value).length===3&&id(value.request_id)&&hash(value.request_fingerprint)&&hash(value.definition_fingerprint);
    function readPending(kind){const key=storageKey(kind);if(!key)throw storageError();const raw=root.sessionStorage.getItem(key),rows=raw===null?[]:JSON.parse(raw);if(!Array.isArray(rows)||rows.length>100||!rows.every(kind==='publication'?validPublication:validMetadata)||new Set(rows.map(row=>row.request_id)).size!==rows.length)throw storageError();return rows;}
    function storePending(rows,kind){const key=storageKey(kind);if(rows.length)root.sessionStorage.setItem(key,JSON.stringify(rows));else root.sessionStorage.removeItem(key);}
    function remember(meta,kind){
      if(!active())throw Object.assign(Error('workflow_draft_session_inactive'),{code:'workflow_draft_session_inactive'});
      if(prior.some(row=>!row.done))throw Object.assign(Error('workflow_draft_recovery_required'),{draftRecoveryRequired:true,publicationRecoveryRequired:prior.some(row=>!row.done&&row.kind==='publication')});
      try{if(!storageReady||!(kind==='publication'?validPublication:validMetadata)(meta))throw storageError();const rows=readPending(kind),existing=rows.find(row=>row.request_id===meta.request_id);if(existing){if(!same(existing,meta))throw storageError();return;}if(rows.length>=100)throw storageError();storePending([...rows,meta],kind);}catch{storageReady=false;throw storageError();}
    }
    function forget(meta,kind){
      if(!active())throw Object.assign(Error('workflow_draft_session_inactive'),{code:'workflow_draft_session_inactive'});
      try{const rows=readPending(kind),existing=rows.find(row=>row.request_id===meta.request_id);if(existing&&!same(existing,meta))throw storageError();storePending(rows.filter(row=>row.request_id!==meta.request_id),kind);}catch{storageReady=false;throw storageError();}
    }
    const generatorMessage=key=>bind(notice,()=>shared('workflow.generator.'+key,({storage_required:'Herstelgegevens kunnen niet worden bewaard. Vernieuw de weergave voordat je opslaat.',saved_metadata:'Conceptopslag bevestigd. De lokale herstelverwijzing kon niet worden verwijderd.'})[key]));
    function feedback(error,key='error'){
      if(error.draftStorage){generatorMessage('storage_required');return;}if(error.draftRecoveryRequired){message(error.publicationRecoveryRequired?'publication_pending':'recovery_pending');return;}
    ...
    const session=(id,revision)=>{let own;own=root.FoundlyWorkflowDraftSession.create({id,revision,request,requestContext,recoverable:recoveryEnabled,beforeRequest:remember,onAcknowledged:forget,onRejected:forget,isActive:()=>active()&&draftSession===own&&draftId===id,onState:state=>state==='SAVED_METADATA'?generatorMessage('saved_metadata'):message({SAVING:'saving',SAVED:'saved',CONFLICT:'conflict',ERROR:'error'}[state])});return own;};
    ...
    if(zeroRequest){
      ...
      zeroSave=button(zeroBox,Object.assign(()=>copy(pendingZero?'retry_zero':'zero_confirm'),{key:'zero_confirm'}),async()=>{
        ...
        if(!pendingZero){if(!reason.value.trim()){message('reason_required');return;}const input={...zeroPreview.input,...(recoveryEnabled?{request_id:root.crypto.randomUUID()}:{})};pendingZero={preview:zeroPreview,turn:root.crypto.randomUUID(),input,action:clone({operation:'SAVE',draft_id:zeroPreview.id,input:{...input,preview_fingerprint:zeroPreview.fingerprint,confirm:true,reason:reason.value.trim()}})};}
        ...
        try{
          if(recoveryEnabled){pending.metadata??={draft_id:preview.id,request_id:pending.input.request_id,expected_revision:pending.input.expected_revision,request_fingerprint:await root.FoundlyWorkflowDraftSession.fingerprint(JSON.stringify(pending.input))};remember(pending.metadata);if(!active())return;}
          const result=await zeroRequest(clone(pending.action),pending.turn);if(!active()||pendingZero!==pending)return;
          await root.FoundlyWorkflowDraftSession.acknowledge(result,{id:preview.id,revision:preview.input.expected_revision,body:JSON.stringify(pending.input),requestContext});if(!active()||pendingZero!==pending)return;
          if(!same(root.FoundlyWorkflowAuthoring.compile(result.record.draft,spec),preview.definition))throw invalid();
          let forgotten=true;if(recoveryEnabled)try{forget(pending.metadata);}catch{forgotten=false;}
          pendingZero=null;lock();
          draftSession=session(draftId,result.record.revision);savedGeneration=generation;form.dataset.unsaved='false';zeroPreview=null;clearPreview();if(forgotten)message('zero_saved');else generatorMessage('saved_metadata');
          zeroSave.disabled=true;
        }catch(error){if(active()&&pendingZero===pending){if(definite(error)){if(recoveryEnabled&&pending.metadata)try{forget(pending.metadata);}catch{error.draftStorage=true;}pendingZero=null;zeroPreview=null;clearPreview();lock();zeroSave.disabled=true;}feedback(error,pendingZero||pendingPublication?'unconfirmed':'error');}}finally{exclusive=false;if(active())lock();}
      });zeroSave.disabled=true;
    }
    ...
    function showRecovery(entry){
      const row={entry,done:false,busy:false},box=make('fieldset'),description=make('p',label('recovery_pending')),output=make('output');output.setAttribute('aria-live','polite');box.append(description,output);form.append(box);prior.push(row);
      const recover=button(box,label('recover'),async()=>{
        if(row.busy||row.done||flushing||pendingZero||pendingPublication)return;row.busy=true;recover.disabled=true;bind(output,()=>copy('recovery_loading'));
        try{
          const result=await request('/api/automation/drafts/'+encodeURIComponent(entry.draft_id)+'/writes/recover',{method:'POST',body:JSON.stringify({request_id:entry.request_id,expected_revision:entry.expected_revision,request_fingerprint:entry.request_fingerprint,confirm:true})});if(!active())return;
          if(!record(result)||!record(result.request_context)||!realm||!['tenant_id','dealer_id','actor_id'].every(key=>result.request_context[key]===realm[key])||result.request_id!==entry.request_id||result.draft_id!==entry.draft_id||result.expected_revision!==entry.expected_revision||result.request_fingerprint!==entry.request_fingerprint||result.executable!==false||!['APPLIED','NOT_APPLIED'].includes(result.state))throw invalid();
          if(result.state==='NOT_APPLIED'){if(result.record!==null)throw invalid();}
          else{const r=result.record;if(!record(r)||r.id!==entry.draft_id||r.revision!==entry.expected_revision+1||r.request_revision!==entry.expected_revision||r.owner_id!==realm.actor_id||r.tenant_id!==realm.tenant_id||r.dealer_id!==realm.dealer_id||r.status!=='DRAFT'||r.executable!==false||r.schema_version!==1||!record(r.draft)||!hash(r.fingerprint)||r.fingerprint!==await root.FoundlyWorkflowDraftSession.fingerprint(JSON.stringify(r.draft)))throw invalid();}
          if(!active())return;row.done=true;bindings.delete(description);description.textContent='';let forgotten=true;try{forget(entry);}catch{forgotten=false;}bind(output,()=>copy(forgotten?(result.state==='APPLIED'?'recovery_saved':'recovery_not_applied'):'recovery_metadata'));if(result.state==='APPLIED')await onDraftRecovered(result.record);
        }catch(error){if(active()&&!row.done){
          if(['workflow_draft_write_superseded','workflow_draft_write_unavailable'].includes(error.code)){row.done=true;bindings.delete(description);description.textContent='';try{forget(entry);}catch{}bind(output,()=>copy('recovery_changed'));}
          else{const status=error.status||error.statusCode;bind(output,()=>[401,403].includes(status)?shared(status===403?'common.access_denied':'identity.auth_required',status===403?'Toegang geweigerd.':'Meld je opnieuw aan.'):copy('unconfirmed'));}
        }}finally{if(active()){row.busy=false;recover.disabled=row.done;}}
      });recover.setAttribute('data-workflow-editor-recovery','recover');
    }
    const publicationShape=row=>({name:row.name,version:row.version,trigger:row.trigger,actions:row.actions,enabled:row.enabled,approval_required:row.approval_required});
    async function verifyPublication(result,expected,signature){
      if(!record(result)||typeof result.id!=='string'||!result.id||result.immutable_version!==true||!hash(result.signature)||typeof result.name!=='string'||!Number.isSafeInteger(result.version)||result.version<1||typeof result.enabled!=='boolean'||typeof result.approval_required!=='boolean'||!record(result.trigger)||!Array.isArray(result.actions)||requestContext&&(result.tenant_id!==requestContext.tenant_id||result.dealer_id!==requestContext.dealer_id))throw invalid();
      if(expected&&(result.name!==expected.name||result.version!==expected.version||result.enabled!==true||result.approval_required!==expected.approval_required||!same(result.actions,expected.actions)||!same(result.trigger,expected.trigger)))throw invalid();
      if(signature&&result.signature!==signature||result.signature!==await root.FoundlyWorkflowDraftSession.fingerprint(JSON.stringify(publicationShape(result))))throw invalid();
    }
    function showPublicationRecovery(entry){
      const row={entry,kind:'publication',done:false,busy:false},box=make('fieldset'),description=make('p',label('publication_pending')),output=make('output');output.setAttribute('aria-live','polite');box.append(description,output);form.append(box);prior.push(row);
      const recover=button(box,label('recover_publication'),async()=>{
        if(row.busy||row.done||flushing||pendingZero||pendingPublication)return;row.busy=true;recover.disabled=true;bind(output,()=>copy('publication_loading'));
        try{
          const result=await request('/api/automation/publications/recover',{method:'POST',body:JSON.stringify({...entry,confirm:true})});if(!active())return;
          if(!record(result)||!record(result.request_context)||!realm||!['tenant_id','dealer_id','actor_id'].every(key=>result.request_context[key]===realm[key])||!Object.keys(entry).every(key=>result[key]===entry[key])||!['APPLIED','NOT_APPLIED'].includes(result.state))throw invalid();
          if(result.state==='NOT_APPLIED'){if(result.record!==null)throw invalid();}else await verifyPublication(result.record,null,entry.definition_fingerprint);
          if(!active())return;row.done=true;bindings.delete(description);description.textContent='';let forgotten=true;try{forget(entry,'publication');}catch{forgotten=false;}bind(output,()=>copy(forgotten?(result.state==='APPLIED'?'publication_recovered':'publication_not_applied'):'publication_recovery_metadata'));if(result.state==='APPLIED'&&form.dataset.unsaved!=='true')await onSaved();
        }catch(error){if(active()&&!row.done){
          if(error.code==='automation_publication_unavailable'){row.done=true;bindings.delete(description);description.textContent='';try{forget(entry,'publication');}catch{}bind(output,()=>copy('publication_unavailable'));}
          else{const status=error.status||error.statusCode;bind(output,()=>[401,403].includes(status)?shared(status===403?'common.access_denied':'identity.auth_required',status===403?'Toegang geweigerd.':'Meld je opnieuw aan.'):copy('unconfirmed'));}
        }}finally{if(active()){row.busy=false;recover.disabled=row.done;}}
      });recover.setAttribute('data-workflow-publication-recovery','recover');
    }
    if(recoveryEnabled)try{for(const entry of readPending())showRecovery(entry);for(const entry of readPending('publication'))showPublicationRecovery(entry);}catch{storageReady=false;generatorMessage('storage_required');}
    form.addEventListener('submit',async event=>{event.preventDefault();if(!active()||exclusive||flushing||pendingZero||publicationKnown)return;exclusive=true;const start=generation,own=draftSession;save.disabled=true;try{
      if(!pendingPublication){const definition=root.FoundlyWorkflowAuthoring.compile(read(),spec);form.inert=true;await flush();if(!active()||generation!==start||draftSession!==own)return;const request_id=recoveryEnabled?root.crypto.randomUUID():null;pendingPublication={definition:clone(definition),request_id,body:JSON.stringify({...definition,...(request_id?{request_id}:{})})};}
      const pending=pendingPublication;form.dataset.unsaved='true';lock();
      if(recoveryEnabled){pending.metadata??={request_id:pending.request_id,request_fingerprint:await root.FoundlyWorkflowDraftSession.fingerprint(pending.body),definition_fingerprint:await root.FoundlyWorkflowDraftSession.fingerprint(JSON.stringify(publicationShape({...pending.definition,enabled:true})))};if(!active())return;remember(pending.metadata,'publication');}
      const result=await request('/api/automation/workflows',{method:'POST',body:pending.body});if(!active()||pendingPublication!==pending)return;
      await verifyPublication(result,pending.definition,pending.metadata?.definition_fingerprint);
      if(recoveryEnabled&&(result.request_id!==pending.request_id||result.request_fingerprint!==pending.metadata.request_fingerprint||!record(result.request_context)||!['tenant_id','dealer_id','actor_id'].every(key=>result.request_context[key]===realm[key])))throw invalid();if(!active()||pendingPublication!==pending)return;
      let forgotten=true;if(recoveryEnabled)try{forget(pending.metadata,'publication');}catch{forgotten=false;}
      pendingPublication=null;publicationKnown=true;form.dataset.unsaved='false';message(forgotten?'published':'publication_metadata');await onSaved();if(active())publicationKnown=false;
    }catch(error){if(active()){if(publicationKnown)message('published_refresh');else{if(definite(error)){if(recoveryEnabled&&pendingPublication?.metadata)try{forget(pendingPublication.metadata,'publication');}catch{error.draftStorage=true;}pendingPublication=null;}feedback(error,pendingPublication?'unconfirmed':'error');}}}finally{exclusive=false;if(active()){lock();if(!pendingPublication&&!publicationKnown)save.disabled=false;}}});
    return form;
  }
