...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.FoundlyAnalysisLoading=api;})(globalThis,()=>{
  async function load(api,params,kpiIds){
    ...
    const caps=new Set(resolution.capabilities),components={},dashboard={kpis:{}},result={dashboard,components,platform:null,connectors:null,automation:null,models_enabled:caps.has('analysis:reports'),events_enabled:caps.has('analysis:events'),cohorts_enabled:caps.has('analysis:events')&&caps.has('analysis:reports'),cohorts_writable:caps.has('analysis:reports')&&Boolean(resolution.tools?.includes('create_report'))};
