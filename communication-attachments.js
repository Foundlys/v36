...
function list(core,ctx,actor,id){
 ...
 return {items:references(core,ctx,id,draft.attachments),current_revision:draft.revision??null,can_write:canWrite,accepted_media_types:['text/plain','application/pdf','image/png','image/jpeg','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','application/vnd.openxmlformats-officedocument.presentationml.presentation'],accepted_extensions:['.txt','.pdf','.png','.jpg','.jpeg','.docx','.xlsx','.pptx'],max_bytes:MAX_BYTES,max_binary_bytes:require('./communication-binary-attachments').MAX_BYTES,max_current_bytes:require('./communication-binary-attachments').MAX_CURRENT_BYTES,binary_requires_local_scan:true,max_current:MAX_CURRENT,external_send:false};
}
