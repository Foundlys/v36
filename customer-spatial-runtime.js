...
(function(root){
  ...
  class CustomerSpatialRuntime {
    ...
    renderPose(){
      ...
      for(const {line,edge} of this.lines){const from=positions.get(edge.from),to=positions.get(edge.to);if(!from||!to)continue;const s=M.segment(from,to);line.style.width=`${s.length}px`;line.style.transform=`translate3d(${from.join('px,')}px) rotateZ(${s.roll}rad) rotateY(${s.yaw}rad)`;}
