...
const PROCUREMENT_OPERATIONS=Object.values(require('./procurement-zero').OPERATIONS);
const MARKETING_OPERATIONS=Object.values(require('./marketing-zero').OPERATIONS);
const FINANCE_OPERATIONS=Object.values(require('./finance-zero').OPERATIONS);
const CALENDAR_OPERATIONS=Object.values(require('./calendar-zero').OPERATIONS);
const ANALYSIS_ACTION_OPERATIONS=Object.values(require('./analysis-actions-zero').OPERATIONS);
const SALES_OPERATIONS=Object.values(require('./sales-zero').OPERATIONS);
const AUTOMATION_DRAFT_OPERATIONS=Object.values(require('./workflow-zero').OPERATIONS);
const TOOL_OPERATIONS=Object.freeze(Object.fromEntries([...PROCUREMENT_OPERATIONS,...MARKETING_OPERATIONS,...FINANCE_OPERATIONS,...CALENDAR_OPERATIONS,...ANALYSIS_ACTION_OPERATIONS,...COMMUNICATION_OPERATIONS,...AUTOMATION_DRAFT_OPERATIONS,...SALES_OPERATIONS].map(op=>[op.tool,op.permission])));
...
const TOOL_MODULES = Object.freeze({
  ...Object.fromEntries(PROCUREMENT_OPERATIONS.map(op=>[op.tool,'procurement'])),
  ...Object.fromEntries(MARKETING_OPERATIONS.map(op=>[op.tool,'marketing'])),
  ...Object.fromEntries(FINANCE_OPERATIONS.map(op=>[op.tool,'finance'])),
  ...Object.fromEntries(CALENDAR_OPERATIONS.map(op=>[op.tool,'calendar'])),
  ...Object.fromEntries(ANALYSIS_ACTION_OPERATIONS.map(op=>[op.tool,'analysis'])),
  ...Object.fromEntries(SALES_OPERATIONS.map(op=>[op.tool,'sales'])),
...
const TOOL_REQUIRED_CAPABILITIES=freeze(Object.fromEntries(Object.entries(TOOL_MODULES).map(([tool,owner])=>{
  ...
  return [tool,[...new Set([TOOL_CAPABILITIES[tool],...(METHOD_CAPABILITIES[owner]?.[method]||[]),...((tool==='analysis_cohort_retention'||ANALYSIS_ACTION_OPERATIONS.some(op=>op.tool===tool))?['analysis:events']:[]),...(PROCUREMENT_OPERATIONS.find(op=>op.tool===tool)?.capabilities||[]),...(MARKETING_OPERATIONS.find(op=>op.tool===tool)?.capabilities||[]),...(FINANCE_OPERATIONS.find(op=>op.tool===tool)?.capabilities||[]),...(CALENDAR_OPERATIONS.find(op=>op.tool===tool)?.capabilities||[]),...(COMMUNICATION_OPERATIONS.find(op=>op.tool===tool)?.capabilities||[])])]];
})));
const MODULES = freeze(Object.fromEntries(Object.entries(DEFINITIONS).map(([id, [label, engine, capabilities, categories]]) => [id, {
  ...
  published_events: EVENT_CONTRACTS[id] || [`${id}.record.created.v1`, `${id}.record.updated.v1`,...(['procurement','sales'].includes(id)?[`${id}.record.approved.v1`]:[])], subscribed_events: id==='automation'?['lead_created','deal_changed','deal_won','deal_lost','crm_record_changed','appointment_scheduled','invoice_paid','connector_state_changed']:[], event_subscription_policy:id==='automation'?{envelope_version:1,activation:'EXPLICIT_WORKFLOW_VERSION_OPT_IN',custom_event_selector:'EXACT_EVENT_NAME',execution_identity:'CURRENT_WORKFLOW_OWNER',permission_context:'CURRENT_PRINCIPAL',idempotency:'WORKFLOW_ID_AND_EVENT_ID'}:null,
