...
const TOOL_OPERATIONS=Object.freeze(Object.fromEntries([...ANALYSIS_ACTION_OPERATIONS,...COMMUNICATION_OPERATIONS,...AUTOMATION_DRAFT_OPERATIONS,...SALES_OPERATIONS].map(op=>[op.tool,op.permission])));
...
const TOOL_REQUIRED_CAPABILITIES=freeze(Object.fromEntries(Object.entries(TOOL_MODULES).map(([tool,owner])=>{
  ...
  return [tool,[...new Set([TOOL_CAPABILITIES[tool],...(METHOD_CAPABILITIES[owner]?.[method]||[]),...((tool==='analysis_cohort_retention'||ANALYSIS_ACTION_OPERATIONS.some(op=>op.tool===tool))?['analysis:events']:[]),...(COMMUNICATION_OPERATIONS.find(op=>op.tool===tool)?.capabilities||[])])]];
})));
