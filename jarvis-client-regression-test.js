...
function startupContext({missing=[]}={}){
  ...
  function element(id='node'){const listeners={};return{id,nodeType:1,childNodes:[],children:[],listeners,value:id==='visualQualitySelect'?'AUTO':id==='preferredAddress'?'Big Boss':'.62',checked:true,textContent:'',innerHTML:'',dataset:{},style:{setProperty(){},removeProperty(){}},classList:{add(){},remove(){},toggle(){},contains(){return false}},addEventListener(type,handler){(listeners[type]||(listeners[type]=[])).push(handler)},append(...nodes){for(const node of nodes){node.parentNode=this;this.childNodes.push(node);if(node.nodeType===1)this.children.push(node)}},appendChild(node){this.append(node)},replaceChildren(...nodes){this.childNodes=[];this.children=[];this.append(...nodes)},setAttribute(){},focus(){},scrollIntoView(){},animate(){},getContext(){return null}}}
  const get=id=>{if(missingIds.has(id))return null;if(!elements.has(id))elements.set(id,element(id));return elements.get(id)},document={hidden:false,body:element('body'),documentElement:element('documentElement'),querySelector(selector){if(selector.startsWith('#'))return get(selector.slice(1));if(selector==='.voicebar')return get('voicebar');return element(selector)},querySelectorAll(){return[]},getElementById:get,createElement:tag=>element(tag),createTextNode:text=>({nodeType:3,textContent:String(text)}),addEventListener(){},exitFullscreen(){}};
