...
require('./communication-imap').createReader=()=>async(config,{authorize,offset})=>{
 const path=process.env.FOUNDLY_IMAP_FIXTURE;if(!path)throw Error('Explicit IMAP fixture path required');const fixture=JSON.parse(fs.readFileSync(path,'utf8'));if(fixture.fixture!=='READ_ONLY_IMAP_CONTRACT_NOT_LIVE')throw Error('Explicit read-only IMAP fixture marker required');authorize();if(fixture.expected_auth_method){require('node:assert/strict').equal(config.auth_method,fixture.expected_auth_method);require('node:assert/strict').equal(config.password,undefined);require('./communication-sasl').credentials(config);}fs.writeFileSync(path+'.started','READ_ONLY_FIXTURE_NO_PROVIDER_MUTATION');
