...
const field=(root,label)=>root.all().find(el=>el.tag==='label'&&(el.textContent===label||el.children.some(child=>child.tag==='span'&&child.textContent===label))).children.find(child=>child.tag==='input');
...
let payload={event:{event_id:'zero-ui:one'},options:{inputs:{flag:true}}},calls=[];const inputHost=new Element('form'),sandbox={document:{createElement:tag=>new Element(tag)},crypto,TextEncoder};vm.createContext(sandbox);vm.runInContext(fs.readFileSync(require.resolve('./workflow-run-zero-client.js'),'utf8'),sandbox);
