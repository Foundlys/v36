...
(async()=>{try{
  ...
  x=await call('/api/jarvis/status');assert.equal(x.r.status,200);assert.equal(x.body.version,'5.0.0');assert.equal(x.body.realtime_configured,true);assert.equal(x.body.realtime.turn_detection,'semantic_vad');assert.equal(x.body.realtime.barge_in,true);assert.equal(x.body.memory.durable,false);assert(x.body.tool_count>=10);assert(x.body.tools.every(tool=>!('handler' in tool)));assert(!JSON.stringify(x.body).includes(serverKey));
  ...
  const html=fs.readFileSync(path.join(__dirname,'index.html'),'utf8'),js=fs.readFileSync(path.join(__dirname,'index-script.js'),'utf8'),ui=html+'\n'+js;assert(!/Yoo bro/i.test(ui));assert(!ui.includes(serverKey));assert(html.includes('<script src="/index-script.js" defer></script>'));assert(!html.includes('<script>'));for(const token of ['RTCPeerConnection','conversation.item.input_audio_transcription.completed','input_audio_buffer.speech_started','response.cancel','UICommandBus','OPEN_ENGINE','SHOW_RESULTS','sessionStorage','processLocally','/api/jarvis/turn'])assert(ui.includes(token),`Jarvis UI contract missing ${token}`);assert(/if\(!rec\|\|!JARVIS\.localWake\)/.test(js),'Ambient wake listener must require local processing');
