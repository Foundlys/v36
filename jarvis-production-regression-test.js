...
(async()=>{try{
  ...
  x=await call('/api/jarvis/status');assert.equal(x.r.status,200);assert.equal(x.body.version,'5.0.2');assert.equal(x.body.realtime_configured,true);assert.equal(x.body.realtime.turn_detection,'semantic_vad');assert.equal(x.body.realtime.barge_in,true);assert.equal(x.body.browser_requirements.wake_detection,'web_speech_recognition_with_on_device_preference');assert.equal(x.body.memory.durable,false);assert(x.body.tool_count>=10);assert(x.body.tools.every(tool=>!('handler' in tool)));assert(!JSON.stringify(x.body).includes(serverKey));
  ...
  console.log(JSON.stringify({ok:true,version:'5.0.2',realtime_ephemeral:'pass',origin_guard:'pass',server_only_key:'pass',semantic_vad:'contract_pass',barge_in:'contract_pass',persistent_wake_preference:'contract_pass',realtime_tool_completion:'contract_pass',text_submit_feedback:'contract_pass',current_search:'pass',follow_up:'pass',tool_registry:'pass',ui_bus:'pass',idempotency:'pass',confirmation_replay:'pass',prompt_injection_defense:'pass',memory_pruning:'pass',restart_persistence:'pass',history_control:'pass',secret_exposure:'pass'},null,2));
