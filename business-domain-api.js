...
function createBusinessDomainApi({domains,platform,context,principal,readBody,sendJson,mailAccount=()=>null,mailSubmissions=()=>null,mailboxes=()=>null,mailOAuth=()=>null,calendarLanguage=null}){
  return async(req,res,url)=>{
    ...
    try{
      if(id==='procurement'&&parts[0]==='rfqs'&&parts.length===3&&parts[2]==='economics-preview'&&req.method==='POST')return sendJson(res,200,require('./procurement-economics').compare(core,ctx,actor,parts[1],await readBody(req)));
      if(id==='procurement'&&parts[0]==='rfqs'&&parts.length===3&&parts[2]==='economics-snapshots'&&req.method==='POST')return sendJson(res,201,require('./procurement-economics').snapshot(core,ctx,actor,parts[1],await readBody(req),{idempotency_key:req.headers['idempotency-key']}));
      if(id==='procurement'&&parts[0]==='outcome-observations'&&parts.length===1&&req.method==='POST')return sendJson(res,201,require('./procurement-outcomes').record(core,ctx,actor,await readBody(req),{idempotency_key:req.headers['idempotency-key']}));
      if(id==='procurement'&&parts[0]==='suppliers'&&parts.length===3&&parts[2]==='outcomes'&&req.method==='GET')return sendJson(res,200,require('./procurement-outcomes').supplier(core,ctx,actor,parts[1]));
      if(id==='marketing'&&parts[0]==='measurement'&&parts.length===2&&['query','drilldown'].includes(parts[1])&&req.method==='POST')return sendJson(res,200,require('./marketing-metrics')[parts[1]](core,platform,ctx,actor,await readBody(req)));
      ...
      if(id==='marketing'&&parts[0]==='creatives'){const history=require('./marketing-creative-history'),options={idempotency_key:req.headers['idempotency-key']};if(parts.length===3&&parts[2]==='revisions'&&req.method==='GET')return sendJson(res,200,{ok:true,...history.history(core,ctx,actor,parts[1],Object.fromEntries(url.searchParams))});if(parts.length===3&&parts[2]==='version'&&req.method==='POST')return sendJson(res,200,{ok:true,...history.save(core,ctx,actor,parts[1],await readBody(req),options)});if(parts.length===5&&parts[2]==='revisions'&&parts[4]==='restore'&&req.method==='POST')return sendJson(res,200,{ok:true,...history.save(core,ctx,actor,parts[1],await readBody(req),options,parts[3])});}
      ...
      if(id==='procurement'){
        ...
        if(parts[0]==='rfqs'&&parts.length===3&&parts[2]==='incremental-allocation-preview'&&req.method==='POST')return sendJson(res,200,{ok:true,...require('./procurement-incremental-allocations').preview(core,ctx,actor,parts[1],(await readBody(req)).allocations)});
