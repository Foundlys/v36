...
function createBusinessDomainApi({domains,platform,context,principal,readBody,sendJson,mailAccount=()=>null,mailSubmissions=()=>null,mailboxes=()=>null,mailOAuth=()=>null,calendarLanguage=null}){
  return async(req,res,url)=>{
    ...
    try{
      ...
      if(id==='calendar'&&parts[0]==='external-calendar'){if(!core.external)return sendJson(res,503,{ok:false,code:'calendar_external_unavailable'});if(parts.length===1&&req.method==='GET')return sendJson(res,200,{ok:true,...core.external.status(ctx,actor,url.searchParams.get('calendar_id'))});if(parts.length===2&&parts[1]==='disable'&&req.method==='POST')return sendJson(res,200,{ok:true,...core.external.disable(ctx,actor,await readBody(req),{idempotency_key:req.headers['idempotency-key']})});if(parts.length===2&&parts[1]==='reconcile'&&req.method==='POST')return sendJson(res,200,{ok:true,...await core.external.reconcile(ctx,actor,await readBody(req),{idempotency_key:req.headers['idempotency-key']})});return sendJson(res,405,{ok:false,code:'method_not_allowed'});}
