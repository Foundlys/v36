...
(function exposeZeroAudio(root,factory){
  const api=factory();
...
})(typeof globalThis!=='undefined'?globalThis:this,function buildZeroAudio(){
  const clamp=(value,min,max)=>Math.max(min,Math.min(max,value));
  const ENERGY_CYCLE_SECONDS=3.15;
  const VOICE_REFERENCE_PROFILE=Object.freeze({analysis_source:'provided_reference_audio',integrated_lufs:-13.7,loudness_range_lu:5,peak_dbfs:.1,median_fundamental_hz:98.8,median_spectral_centroid_hz:814,stereo_side_mid_ratio:.255,target:'adult_low_mid_controlled_cinematic_subtly_synthetic',selected_builtin_voice:'cedar',exact_reference_voice_identified:false,selection_basis:'closest_legitimate_builtin_quality_profile_plus_measured_post_processing',post_processing:{high_pass_hz:64,body_hz:185,body_gain_db:2.8,warmth_hz:430,presence_hz:2450,air_hz:6800,compression_ratio:3,reverb_seconds:1.45,wet_gain:.085}});
  const SOUNDTRACK_STATE={STANDBY:.62,ACTIVATING:1,LISTENING:.34,THINKING:.72,SEARCHING:.86,PLANNING:.76,WAITING_TOOL:.58,EXECUTING:.92,WAITING_CONFIRMATION:.46,VERIFYING:.67,SPEAKING:.24,SUCCESS:.78,WARNING:.42,ERROR:.26,RECOVERING:.56,MUTED:0};
  function deterministicNoise(seed=0x5f3759df){let value=seed>>>0;return()=>{value=(value*1664525+1013904223)>>>0;return value/4294967296*2-1}}
  ...
  class FoundlyAudioScene{
    constructor({onLevel,onCapability,onStatus}={}){
      this.onLevel=onLevel||(()=>{});this.onCapability=onCapability||(()=>{});this.onStatus=onStatus||(()=>{});this.context=null;this.ready=false;this.outputUnlocked=false;this.userSpeaking=false;this.zeroSpeaking=false;this.state='STANDBY';this.levelFrame=0;this.remoteSource=null;this.outputLevel=0;this.voiceLevel=0;this.cueActiveUntil=0;
    ...
    buildOriginalSoundscape(){
      const c=this.context,noiseRandom=deterministicNoise();this.oscillators=[];this.ambientFilter=c.createBiquadFilter();this.ambientFilter.type='lowpass';this.ambientFilter.frequency.value=420;this.ambientFilter.Q.value=.8;this.ambientFilter.connect(this.music);
      for(const [frequency,gainValue,type,pan] of [[43.65,.028,'sine',-.58],[65.41,.072,'triangle',.52],[98,.020,'sine',-.16],[130.81,.012,'sine',.7],[329.63,.0042,'sine',-.36],[523.25,.0024,'sine',.38]]){const oscillator=c.createOscillator(),gain=c.createGain(),panner=c.createStereoPanner?c.createStereoPanner():null;oscillator.type=type;oscillator.frequency.value=frequency;gain.gain.value=gainValue;oscillator.connect(gain);if(panner){panner.pan.value=pan;gain.connect(panner);panner.connect(this.ambientFilter)}else gain.connect(this.ambientFilter);oscillator.start();this.oscillators.push({oscillator,gain,panner,base:frequency,baseGain:gainValue})}
      this.breathOsc=c.createOscillator();this.breathDepth=c.createGain();this.breathOsc.type='sine';this.breathOsc.frequency.value=1/ENERGY_CYCLE_SECONDS;this.breathDepth.gain.value=88;this.breathOsc.connect(this.breathDepth);this.breathDepth.connect(this.ambientFilter.frequency);this.breathOsc.start();
      this.pulseOsc=c.createOscillator();this.pulseGain=c.createGain();this.pulseFilter=c.createBiquadFilter();this.pulseLfo=c.createOscillator();this.pulseDepth=c.createGain();this.pulseOsc.type='sine';this.pulseOsc.frequency.value=174.61;this.pulseFilter.type='bandpass';this.pulseFilter.frequency.value=740;this.pulseFilter.Q.value=1.2;this.pulseGain.gain.value=.003;this.pulseLfo.type='sine';this.pulseLfo.frequency.value=1/ENERGY_CYCLE_SECONDS;this.pulseDepth.gain.value=.0018;this.pulseLfo.connect(this.pulseDepth);this.pulseDepth.connect(this.pulseGain.gain);this.pulseOsc.connect(this.pulseFilter);this.pulseFilter.connect(this.pulseGain);this.pulseGain.connect(this.music);this.pulseOsc.start();this.pulseLfo.start();
      const length=Math.max(1,Math.floor(c.sampleRate*ENERGY_CYCLE_SECONDS)),buffer=c.createBuffer(2,length,c.sampleRate);for(let channel=0;channel<2;channel++){const data=buffer.getChannelData(channel);let last=0;for(let i=0;i<length;i++){last=last*.992+noiseRandom()*.008;const envelope=.72+.28*Math.sin((i/length)*Math.PI*2+channel*.9);data[i]=last*.55*envelope}}const noise=c.createBufferSource(),noiseFilter=c.createBiquadFilter(),noiseGain=c.createGain();noise.buffer=buffer;noise.loop=true;noiseFilter.type='bandpass';noiseFilter.frequency.value=1250;noiseFilter.Q.value=.38;noiseGain.gain.value=.052;noise.connect(noiseFilter);noiseFilter.connect(noiseGain);noiseGain.connect(this.music);noise.start();this.noise={source:noise,filter:noiseFilter,gain:noiseGain};
      this.shimmer=c.createOscillator();this.shimmerGain=c.createGain();this.shimmerPan=c.createStereoPanner?c.createStereoPanner():null;this.shimmer.type='sine';this.shimmer.frequency.value=1046.5;this.shimmerGain.gain.value=.0032;this.shimmer.connect(this.shimmerGain);if(this.shimmerPan){this.shimmerPan.pan.value=.28;this.shimmerGain.connect(this.shimmerPan);this.shimmerPan.connect(this.music)}else this.shimmerGain.connect(this.music);this.shimmer.start();
    }
    ...
    setZeroSpeaking(active){this.zeroSpeaking=Boolean(active);this.applyMix()}
    setJarvisSpeaking(active){return this.setZeroSpeaking(active)}
    applyMix(){
      if(!this.ready)return;const now=this.context.currentTime,stateFactor=SOUNDTRACK_STATE[this.state]??SOUNDTRACK_STATE.STANDBY,ducked=this.userSpeaking||this.zeroSpeaking,drive={ACTIVATING:1,THINKING:.72,SEARCHING:1,PLANNING:.78,EXECUTING:1.18,VERIFYING:.66,SUCCESS:.62,WARNING:.48,ERROR:.18,RECOVERING:.55}[this.state]??.28;
      this.master.gain.setTargetAtTime(this.settings.muted?0:this.settings.master,now,.035);this.voice.gain.setTargetAtTime(this.settings.voiceEnabled?this.settings.voice:0,now,.035);this.sfx.gain.setTargetAtTime(this.settings.sfxEnabled?this.settings.sfx:0,now,.035);this.music.gain.setTargetAtTime(this.settings.musicEnabled?this.settings.music*stateFactor:0,now,ducked?.055:.45);this.musicDuck.gain.setTargetAtTime(ducked?.58:1,now,ducked?.065:.42);
      this.ambientFilter.frequency.setTargetAtTime(330+drive*410,now,.38);this.breathDepth.gain.setTargetAtTime(58+drive*58,now,.5);this.pulseGain.gain.setTargetAtTime(this.settings.musicEnabled?(.0025+drive*.0215):0,now,.3);this.pulseDepth.gain.setTargetAtTime(.0012+drive*.0021,now,.4);this.pulseOsc.frequency.setTargetAtTime(this.state==='EXECUTING'?220:this.state==='SEARCHING'?196:174.61,now,.28);this.noise.filter.frequency.setTargetAtTime(this.state==='SEARCHING'?1780:this.state==='EXECUTING'?1480:980+drive*360,now,.5);this.noise.gain.gain.setTargetAtTime(.025+drive*.034,now,.55);this.shimmerGain.gain.setTargetAtTime(.0015+drive*.0042,now,.6);for(const [index,row] of this.oscillators.entries())row.gain.gain.setTargetAtTime(row.baseGain*(.82+drive*(index===0?.36:.22)),now,.7);this.emitStatus();
    }
    attachRemote(stream){if(!this.ready||!stream)return false;try{this.remoteSource?.disconnect()}catch{}const c=this.context,source=c.createMediaStreamSource(stream),high=c.createBiquadFilter(),body=c.createBiquadFilter(),warmth=c.createBiquadFilter(),presence=c.createBiquadFilter(),air=c.createBiquadFilter();high.type='highpass';high.frequency.value=64;body.type='lowshelf';body.frequency.value=185;body.gain.value=2.8;warmth.type='peaking';warmth.frequency.value=430;warmth.Q.value=.72;warmth.gain.value=1.1;presence.type='peaking';presence.frequency.value=2450;presence.Q.value=.76;presence.gain.value=.8;air.type='highshelf';air.frequency.value=6800;air.gain.value=-1.3;source.connect(high);high.connect(body);body.connect(warmth);warmth.connect(presence);presence.connect(air);let tail=air;const nodes=[high,body,warmth,presence,air];if(c.createWaveShaper){const saturator=c.createWaveShaper(),curve=new Float32Array(257);for(let i=0;i<curve.length;i++){const x=i/(curve.length-1)*2-1;curve[i]=Math.tanh(x*1.12)/Math.tanh(1.12)}saturator.curve=curve;saturator.oversample='2x';tail.connect(saturator);tail=saturator;nodes.push(saturator)}const dry=c.createGain();dry.gain.value=.96;tail.connect(dry);dry.connect(this.voice);nodes.push(dry);if(c.createConvolver){const reverb=c.createConvolver(),wet=c.createGain(),random=deterministicNoise(0x4a415256),length=Math.max(1,Math.floor(c.sampleRate*1.45)),impulse=c.createBuffer(2,length,c.sampleRate);for(let channel=0;channel<2;channel++){const values=impulse.getChannelData(channel);for(let i=0;i<length;i++){const decay=Math.pow(1-i/length,2.6);values[i]=random()*decay*(channel?.84:.9)}}reverb.buffer=impulse;wet.gain.value=.085;tail.connect(reverb);reverb.connect(wet);wet.connect(this.voice);nodes.push(reverb,wet)}this.remoteSource=source;this.remoteNodes=nodes;return true}
    ...
    canDetectWake(){return !this.zeroSpeaking&&!this.userSpeaking&&performance.now()>this.cueActiveUntil}
    ...
    sampleLevel(){if(!this.ready)return;const outputValues=this.outputValues,voiceValues=this.voiceValues;this.outputLevel=this.sampleBand(this.outputAnalyser,outputValues);this.voiceLevel=this.sampleBand(this.voiceAnalyser,voiceValues);let low=0,mid=0,high=0;for(let i=0;i<voiceValues.length;i++){const v=voiceValues[i]/255;if(i<voiceValues.length*.12)low+=v;else if(i<voiceValues.length*.48)mid+=v;else high+=v}low/=Math.max(1,Math.floor(voiceValues.length*.12));mid/=Math.max(1,Math.floor(voiceValues.length*.36));high/=Math.max(1,Math.floor(voiceValues.length*.52));if(!this.zeroSpeaking){low=0;mid=0;high=0}const level=this.zeroSpeaking?clamp(low*.3+mid*.54+high*.16,0,1):0;this.onLevel({level,low,mid,high,voice:this.zeroSpeaking?this.voiceLevel:0,output:this.outputLevel,speaking:this.zeroSpeaking});this.levelFrame=requestAnimationFrame(()=>this.sampleLevel())}
    diagnostics(extra={}){const musicGain=this.music?.gain?.value||0;return {assistant:'ZERO',audio_context_state:this.context?.state||'uninitialized',output_unlocked:Boolean(this.outputUnlocked),music_enabled:Boolean(this.settings.musicEnabled),music_playing:Boolean(this.ready&&this.outputUnlocked&&musicGain>.002&&!this.settings.muted),music_gain:Number(musicGain.toFixed(3)),voice_enabled:Boolean(this.settings.voiceEnabled),voice_active:Boolean(this.zeroSpeaking),ducking_active:Boolean(this.userSpeaking||this.zeroSpeaking),sfx_enabled:Boolean(this.settings.sfxEnabled),muted:Boolean(this.settings.muted),output_level:Number(this.outputLevel.toFixed(3)),voice_level:Number(this.voiceLevel.toFixed(3)),soundscape_profile:'original_foundly_neural_3_15',voice_reference_profile:VOICE_REFERENCE_PROFILE,audio_reactive_source:'web_audio_frequency_analysis',...extra}}
    emitStatus(extra={}){const status=this.diagnostics(extra);this.onStatus(status);return status}
    dispose(){if(this.levelFrame)cancelAnimationFrame(this.levelFrame);try{this.remoteSource?.disconnect();for(const row of this.oscillators||[])row.oscillator.stop();this.noise?.source.stop();this.pulseOsc?.stop();this.pulseLfo?.stop();this.breathOsc?.stop();this.shimmer?.stop();this.context?.close()}catch{}this.ready=false;this.outputUnlocked=false;this.emitStatus()}
  }

  return {DoubleClapGate,FoundlyClapDetector,FoundlyAudioScene,SOUNDTRACK_STATE,ENERGY_CYCLE_SECONDS,VOICE_REFERENCE_PROFILE};
});
