'use strict';
const crypto=require('node:crypto'),{scopedMutation}=require('./scoped-mutation'),{assertCorePermission}=require('./core-access-contracts'),{smtpConfigurationState,configurationReady}=require('./communication-provider-state');
...
class MailAuthentication{
 ...
 async probe(ctx,actor){
  ...
  const authorize=()=>{this.authorize(ctx,actor);require('./communication-sasl').current(config);if(hash(this.configuration(ctx))!==fingerprint)fail('mail_authentication_configuration_changed');if(rows[0]?.attempt_id!==attempt)fail('mail_authentication_attempt_changed');};
  try{
