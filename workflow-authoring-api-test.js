...
(async()=>{try{
 ...
 await stop();env.FOUNDLY_PLATFORM_ROLES='VIEWER';await start();assert.equal((await call('/api/automation/status')).body.can_manage,false);assert.equal((await call('/api/automation/workflows','POST',{...definition,version:3})).status,403);
 console.log('PASS authenticated authoring assets, compiled multistep API, exact approval, delay, encrypted restart, preserved versions, no duplicate side effects and viewer write denial');
