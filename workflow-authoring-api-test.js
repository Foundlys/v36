...
(async()=>{try{
 ...
 for(const when of [false,null,0,'',{any:[leaf,false]},{all:[]},{all:[leaf],any:[leaf]},{field:'inputs.score',operator:'ne'}]){
   const rejected=await call('/api/automation/workflows','POST',{name:'Malformed HTTP condition',trigger:'custom_event',actions:[{type:'create_task',title:'Must never exist',when}]});assert.equal(rejected.status,422,JSON.stringify(rejected.body));assert.equal(rejected.body.code,'automation_condition_invalid');
 }
 ...
 const definition=compile({name:'HTTP authoring fixture',version:1,trigger_type:'custom_event',approval_required:true,steps:[{type:'create_task',values:{title:'Authoring task'},condition:{enabled:true,mode:'all',children:[{field:'inputs.score',operator:'gte',value_type:'number',value:'5'},{mode:'any',children:[{field:'inputs.score',operator:'lt',value_type:'number',value:'9'},{field:'inputs.flag',operator:'eq',value_type:'boolean',value:'true'}]}]}},{type:'delay',values:{seconds:1}},{type:'create_document',values:{title:'Authoring document',content:'Exact reviewed content'}}]},status.editor_contract);
 const partial={name:'Nested partial HTTP',steps:[{type:'create_task',values:{title:''},condition:{enabled:true,mode:'all',children:[{mode:'any',children:[{mode:'all',children:[{field:'inputs.score',operator:'gte',value_type:'number',value:'5'}]}]}]}}]};
 const savedDraft=await call('/api/automation/drafts/grouped-condition-fixture','PUT',{draft:partial,expected_revision:0});assert.equal(savedDraft.status,200,JSON.stringify(savedDraft.body));
 await stop();await start();const retainedDraft=(await call('/api/automation/drafts')).body.items.find(row=>row.id==='grouped-condition-fixture');assert.deepEqual(retainedDraft.draft,partial);assert.equal((await call('/api/automation/status')).body.workflow_count,0);
 ...
 const skipEvent={event_id:'authoring-http:group-false',event_version:1};let skipped=(await call(route,'POST',{event:skipEvent,options:{inputs:{score:10}}})).body;
 assert.equal(skipped.steps[0].status,'SKIPPED_CONDITION');assert.equal((await call('/api/automation/tasks')).body.total,1);
 ...
 const awaitingManual=await call(route,'POST',manual);assert.equal(awaitingManual.status,202);assert.equal(awaitingManual.body.status,'AWAITING_APPROVAL');assert.equal(awaitingManual.body.steps[0].status,'SKIPPED_CONDITION');assert.equal(awaitingManual.body.steps[1].status,'SUCCEEDED');assert.equal((await call('/api/automation/documents')).body.total,1);
