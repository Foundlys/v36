...
function update(core,ctx,actor,id,input,options={}){
 ...
 const recovery=require('./communication-inbox-recovery'),meta=recovery.metadata(id,input,options.idempotency_key),key=hash(['INBOX_STATE',options.idempotency_key]),fingerprint=meta.request_fingerprint,matches=operations.filter(row=>row.key===key&&row.actor_id===actor.id);if(matches.length>1)fail('inbox_action_unverifiable','De persoonlijke inboxactie is niet verifieerbaar',409);const seen=matches[0];if(seen?.receipt_version===1)return recovery.replay(core,ctx,actor,meta,seen);
 ...
 const result=core.mutate(ctx,()=>{
  ...
  if(prior)rows[rows.indexOf(prior)]=row;else rows.push(row);const receipt={message_id:id,result_revision:row.revision,source_revision:source.revision,at:now};const request_acknowledgement=recovery.store(core,ctx,actor,meta,input,row,prior,receipt);
  core.recordEvent(ctx,actor,'inbox_state',row,'updated');return {record:clone(row),...acknowledgement(receipt),local_state:stateView(row,source.revision),receipt,deduplicated:false,provider_updated:false,action_executed:true,request_acknowledgement};
