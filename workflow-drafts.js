...
const SCOPE='platform:automation_drafts',REQUESTS='platform:automation_draft_requests',WRITES='platform:automation_draft_writes',REQUEST_LIMIT=10000;
...
class WorkflowDrafts{
  ...
  save(ctx,actor,id,input,origin={}){
    ...
    if(!input||typeof input!=='object'||Array.isArray(input))fail('workflow_draft_invalid','Kies geldige conceptinvoer');
    const requestId=Object.hasOwn(input,'request_id')?input.request_id:null;
    if(Object.hasOwn(input,'request_id')&&(!validId(requestId)||!Number.isSafeInteger(input.expected_revision)||input.expected_revision<0))fail('workflow_draft_recovery_invalid','Ongeldige opslagreferentie');
    ...
    const requestFingerprint=crypto.createHash('sha256').update(JSON.stringify({draft:input.draft,expected_revision:input.expected_revision,...(requestId?{request_id:requestId}:{})})).digest('hex');
    const writes=requestId?this.adapter.bucket(ctx,WRITES):[],write=writes.find(row=>row.actor_id===actor.id&&row.request_id===requestId);
    if(write){
      if(write.draft_id!==id||write.expected_revision!==input.expected_revision||write.request_fingerprint!==requestFingerprint)fail('workflow_draft_write_conflict','Deze opslagreferentie hoort bij andere invoer',409);
      if(write.state==='NOT_APPLIED')fail('workflow_draft_write_abandoned','Deze opslagaanvraag is afgesloten',409);
      if(write.state!=='APPLIED')fail('workflow_draft_write_unverifiable','Deze opslaguitkomst kan niet worden bevestigd',409);
      if(!previous)fail('workflow_draft_write_unavailable','Het bewaarde concept is niet meer beschikbaar',409);
      if(previous.revision!==write.record_revision||previous.fingerprint!==write.record_fingerprint)fail('workflow_draft_write_superseded','Het concept heeft een nieuwere revisie',409);
    }
    ...
    const result=(row,deduplicated)=>({record:clone(row),deduplicated,executable:false,request_context:requestContext(ctx,actor),draft_id:id,expected_revision:input.expected_revision,request_fingerprint:requestFingerprint,...(requestId?{request_id:requestId}:{}),preview_fingerprint:origin.preview_fingerprint||null});
    const retain=row=>{if(requestId)writes.push({request_id:requestId,actor_id:actor.id,draft_id:id,expected_revision:input.expected_revision,request_fingerprint:requestFingerprint,state:'APPLIED',record_revision:row.revision,record_fingerprint:row.fingerprint,recorded_at:new Date().toISOString()});};
    if(write)return result(previous,true);
    if(requestId&&writes.length>=REQUEST_LIMIT)fail('workflow_draft_write_capacity','Het maximumaantal opslagreferenties is bereikt',507);
    if(previous&&previous.request_revision===input.expected_revision&&previous.fingerprint===fingerprint){
      if(!requestId)return result(previous,true);
      return scopedMutation(this.adapter,ctx,[WRITES,'platform:audit'],()=>{retain(previous);this.adapter.audit(ctx,actor,'CONFIRM','automation_draft_write',id,{revision:previous.revision,content_logged:false});return result(previous,true);});
    }
    ...
    return scopedMutation(this.adapter,ctx,[SCOPE,REQUESTS,...(requestId?[WRITES]:[]),'platform:audit'],()=>{
  ...
  recoverWrite(ctx,actor,id,input){
    this.scope(ctx,actor,'write');
    if(!validId(id)||!input||typeof input!=='object'||Array.isArray(input)||Object.keys(input).some(key=>!['request_id','expected_revision','request_fingerprint','confirm'].includes(key))||!validId(input.request_id)||!Number.isSafeInteger(input.expected_revision)||input.expected_revision<0||!validFingerprint(input.request_fingerprint)||input.confirm!==true)fail('workflow_draft_recovery_invalid','Bevestig de exacte conceptopslag');
    ...
    const writes=this.adapter.bucket(ctx,WRITES),write=writes.find(row=>row.actor_id===actor.id&&row.request_id===input.request_id),envelope={request_id:input.request_id,draft_id:id,expected_revision:input.expected_revision,request_fingerprint:input.request_fingerprint,request_context:requestContext(ctx,actor),executable:false};
    if(write){
      if(write.draft_id!==id||write.expected_revision!==input.expected_revision||write.request_fingerprint!==input.request_fingerprint)fail('workflow_draft_write_conflict','Deze herstelreferentie hoort bij andere invoer',409);
      if(write.state==='NOT_APPLIED')return {...envelope,state:'NOT_APPLIED',record:null};
      if(write.state!=='APPLIED')fail('workflow_draft_write_unverifiable','Deze opslaguitkomst kan niet worden bevestigd',409);
      if(!row)fail('workflow_draft_write_unavailable','Het bewaarde concept is niet meer beschikbaar',409);
      if(row.revision!==write.record_revision||row.fingerprint!==write.record_fingerprint)fail('workflow_draft_write_superseded','Het concept heeft een nieuwere revisie',409);
      return {...envelope,state:'APPLIED',record:clone(row)};
    }
    if(!row&&input.expected_revision>0)fail('workflow_draft_write_unavailable','Het eerdere concept is niet meer beschikbaar',409);
    if(row&&row.revision!==input.expected_revision)fail('workflow_draft_write_unverifiable','Er is geen bewaarde bevestiging voor deze eerdere opslag',409);
    if(writes.length>=REQUEST_LIMIT)fail('workflow_draft_write_capacity','Het maximumaantal opslagreferenties is bereikt',507);
    return scopedMutation(this.adapter,ctx,[WRITES,'platform:audit'],()=>{writes.push({request_id:input.request_id,actor_id:actor.id,draft_id:id,expected_revision:input.expected_revision,request_fingerprint:input.request_fingerprint,state:'NOT_APPLIED',recorded_at:new Date().toISOString()});this.adapter.audit(ctx,actor,'CLOSE','automation_draft_write',id,{expected_revision:input.expected_revision,content_logged:false});return {...envelope,state:'NOT_APPLIED',record:null};});
  }
...
function createWorkflowDraftApi({drafts,context,principal,readBody,sendJson}){return async(req,res,url)=>{
  ...
  try{const ctx=context(),actor=principal();if(req.method==='GET'&&['/api/automation/drafts','/api/automation/drafts/export'].includes(url.pathname))return sendJson(res,200,drafts.list(ctx,actor,url.pathname.endsWith('/export')?'export':'read'));
    const writeRecovery=url.pathname.match(/^\/api\/automation\/drafts\/([A-Za-z0-9_-]{1,100})\/writes\/recover$/);if(writeRecovery&&req.method==='POST')return sendJson(res,200,drafts.recoverWrite(ctx,actor,writeRecovery[1],await readBody(req)));
...
module.exports={WorkflowDrafts,createWorkflowDraftApi,SCOPE,REQUESTS,WRITES};
