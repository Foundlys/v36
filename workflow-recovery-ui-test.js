...
const source=fs.readFileSync(require.resolve('./foundly-workspace.js'),'utf8'),start=source.indexOf('        if(data.can_manage&&row.can_recover&&row.steps?.some('),end=source.indexOf("        if(['WAITING_TIME','WAITING_RETRY','RECOVERY_READY'].includes(row.status)",start);assert.ok(start>0&&end>start);
let renders=0,requests=[];
function create(eventId){const row=core.runAutomation(ctx,actor,workflow.id,{event_id:eventId}),card=new Element('article');row.can_recover=true;const sandbox={current:()=>true,card,row,data:{can_manage:true,retryable_actions:['create_task']},section:'FAILURES',content:new Element('div'),node:(tag,cls,text)=>{const e=new Element(tag);e.textContent=text||'';return e;},friendlyError:e=>e.message,renderAutomationSection:()=>{renders++;},read:async(route,options)=>{requests.push({route,options});if(options){const input=JSON.parse(options.body);return core.recoverAutomation(ctx,actor,row.run_id,input);}return core.previewAutomationRecovery(ctx,actor,row.run_id);}};vm.createContext(sandbox);vm.runInContext('(function(){'+source.slice(start,end)+'})()',sandbox);return {row,card};}
(async()=>{
