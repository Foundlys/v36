...
function invalidateCrmAccess(error){
 state.crmAccessGeneration++;state.automationEditor?.invalidate();
...
function crmFormControls(form,locked,busy){for(const node of form.querySelectorAll('input,select,textarea'))node.disabled=locked;const button=[...form.querySelectorAll('button')].find(node=>node.type==='submit')||form.querySelector('button');if(button)button.disabled=busy;if(form===$('#automationForm'))state.automationEditor?.setDisabled(locked);}
...
function initAutomationEditor(){if(!state.automationEditor){state.automationEditor=globalThis.FoundlyCrmAutomationEditor.create({document,i18n:globalThis.FoundlyI18n,request:api,schema:loadSchema});$('#automationActionFields').replaceChildren(state.automationEditor);}return state.automationEditor;}
async function selectAutomationAction(){if(state.formWrites.automation)return;return initAutomationEditor().select($('#automationForm').elements.action.value);}
async function saveAutomation(event){
 event.preventDefault();const form=event.currentTarget;
 return saveCrmForm('automation',form,()=>{const data=Object.fromEntries(new FormData(form).entries());return {path:'/api/crm/automations',request:Object.freeze({method:'POST',headers:Object.freeze({'idempotency-key':`automation-${crypto.randomUUID()}`}),body:JSON.stringify({name:data.name,enabled:true,trigger:{type:data.trigger},actions:[{...(['assign','field_update','stage_change'].includes(data.action)?initAutomationEditor().read(data.action):{type:data.action}),title:String(crmCopy('automation_follow_up','{name} · opvolging',{name:data.name}))}]})})};},async()=>{form.reset();state.automationEditor?.clear();await loadAutomations();toast(crmCopy('automation_saved','Automation opgeslagen'));},crmCopy('automation_saved','Automation opgeslagen'));
}
