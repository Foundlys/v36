...
async function mount({write=true,corrupt=false,binary=false}={}){
 ...
 const bytes=binary?Buffer.from('%PDF-1.4\n%%EOF\n'):Buffer.from('<script>fixture only</script>\nCafé 😀'),attachment={id:'file1',name:binary?'Document.pdf':'Notes.txt',...(binary?{validation:'SIGNATURE_AND_LOCAL_CLAMAV',media_type:'application/pdf',scan_observed_at:'2026-09-11T00:00:00Z'}:{}),size_bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),content_base64:bytes.toString('base64')};
...
(async()=>{
 ...
 const form=find(test.cell,'form'),file=find(form,'input');file.files=[{name:'Upload.txt',size:3,arrayBuffer:async()=>Buffer.from('one')}];test.failUpload(true);await form.fire('submit');const first=test.requests.at(-1);assert.equal(JSON.parse(first.options.body).expected_revision,3);await form.fire('submit');assert.equal(test.requests.at(-1).options.headers['idempotency-key'],first.options.headers['idempotency-key'],'Same input retries keep their idempotency key');
 file.files=[{name:'Changed.txt',size:3,arrayBuffer:async()=>Buffer.from('two')}];test.failUpload(false);await form.fire('submit');assert.notEqual(test.requests.at(-1).options.headers['idempotency-key'],first.options.headers['idempotency-key'],'Changing a failed upload starts a new input-bound request');assert.equal(test.renders(),1);
 ...
 const scanned=await mount({binary:true});await find(scanned.cell,'button','Bestand downloaden').fire('click');assert.equal(scanned.blobs[0].type,'application/octet-stream');assert.equal(scanned.downloads[0].name,'Document.pdf');assert.equal(Buffer.from(await scanned.blobs[0].arrayBuffer()).equals(scanned.bytes),true);assert.ok(scanned.cell.all().some(row=>row.textContent.includes('geen veiligheidsgarantie')));assert.ok(find(find(scanned.cell,'form'),'input').accept.includes('.pdf'));
 const detachedBefore=test.requests.length;test.content.isConnected=false;await form.fire('submit');assert.equal(test.requests.length,detachedBefore,'Detached uploads do not send requests');
 console.log('PASS production attachment handlers verify bytes, download text and scanned binary as inert files, avoid inline rendering, preserve input-bound retry/CAS, bound uploads and respect read-only/detached state; browser acceptance not claimed');
