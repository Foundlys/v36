...
function snapshotReadable(domain,ctx,actor,row){
  ...
  if(row.forecast.hierarchy){const reference=row.forecast.hierarchy,current=domain.bucket(ctx,'forecast_hierarchies').find(r=>r.id===reference.id&&!r.deleted_at);if(!current||!domain.visible(current,actor)||current.revision!==reference.revision||current.definition_fingerprint!==reference.definition_fingerprint)return false;}
