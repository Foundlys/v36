...
const env={...process.env,NODE_ENV:'production',PORT:String(PORT),FOUNDLY_DATA_DIR:tmp,FOUNDLY_PUBLIC_BASE_URL:base,FOUNDLY_ADMIN_USERNAME:'foundly',FOUNDLY_ADMIN_PASSWORD:password,FOUNDLY_ENCRYPTION_KEY:'security-encryption-key',FOUNDLY_CONNECTOR_ALLOWED_HOSTS:'api.example.com',OPENAI_API_KEY:'',FOUNDLY_AI_API_KEY:'',META_APP_ID:'',META_APP_SECRET:'',GOOGLE_CLIENT_ID:'',GOOGLE_CLIENT_SECRET:'',FOUNDLY_WORKER_INTERVAL_MS:'99999999'};
...
(async()=>{try{for(let i=0;i<60;i++){try{if((await fetch(base+'/api/health')).ok)break}catch{}await wait(100)}
  ...
  x=await call('/api/connector-runtime/profiles',{method:'POST',headers:{authorization:auth,'content-type':'application/json'},body:JSON.stringify({id:'ssrf',naam:'SSRF',base_url:'http://127.0.0.1:1',auth_strategy:'public'})});assert.equal(x.r.status,500);
  x=await call('/api/ready');assert.equal(x.r.status,503);assert(x.data.failed.includes('persistent_mount'));
  x=await call('/api/engine/crm/execute',{method:'POST',headers:{authorization:auth,'content-type':'application/json'},body:JSON.stringify({action:'unknown_job',title:'retry test'})});assert.equal(x.r.status,202);
  ...
  console.log(JSON.stringify({ok:true,authentication:'pass',secret_exposure:'pass',tenant_header_isolation:'pass',ssrf_guard:'pass',readiness:'pass',worker_retry:'pass'},null,2));
