...
(async()=>{try{for(let i=0;i<60;i++){try{if((await fetch(base+'/api/health')).ok)break}catch{}await wait(100)}
  ...
  x=await call('/api/connect/meta',{headers:{authorization:auth},redirect:'manual'});assert.equal(x.r.status,503);assert.equal(x.data.code,'oauth_state_store_not_durable');
