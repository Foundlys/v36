...
async function main(){
  await start();
  let x=await req('/');assert.equal(x.r.status,200);assert(String(x.text).includes('Foundly OS v5.0.3'));assert.equal(x.r.headers.get('content-security-policy').includes("script-src 'self'"),true);
  ...
  x=await req('/api/health');assert.equal(x.r.status,200);assert.equal(x.data.version,'5.0.3');assert(!('data_dir'in x.data));
  x=await req('/api/diagnostics/config');assert.equal(x.r.status,200);assert.equal(x.data.version,'5.0.3');assert.equal(x.data.oauth_state.mode,'hashed_hmac_bound_persistent_transaction');assert.equal(x.data.encryption.configured,true);assert(!JSON.stringify(x.data).includes('test-secret'));assert(!JSON.stringify(x.data).includes('secret_fingerprint'));
  ...
  console.log(JSON.stringify({ok:true,version:'5.0.3',oauth_state:'pass',persistence:'pass',worker_recovery:'pass',orchestration:'pass',runtime_connectors:'pass',source_contracts:'pass',provenance:'pass',ui_actions:'pass',jarvis_ui_contract:'pass',base_connectors:93},null,2));
}
