...
async function main(){
  await start();
  let x=await req('/');assert.equal(x.r.status,200);assert(String(x.text).includes('Foundly OS v5.1.0'));assert.equal(x.r.headers.get('content-security-policy').includes("script-src 'self'"),true);
  ...
  for(const asset of ['/jarvis-audio.js','/neural-runtime.js']){x=await req(asset);assert.equal(x.r.status,200);assert((x.r.headers.get('content-type')||'').includes('application/javascript'))}
  x=await req('/api/health');assert.equal(x.r.status,200);assert.equal(x.data.version,'5.1.0');assert(!('data_dir'in x.data));
  x=await req('/api/diagnostics/config');assert.equal(x.r.status,200);assert.equal(x.data.version,'5.1.0');assert.equal(x.data.oauth_state.mode,'hashed_hmac_bound_persistent_transaction');assert.equal(x.data.encryption.configured,true);assert(!JSON.stringify(x.data).includes('test-secret'));assert(!JSON.stringify(x.data).includes('secret_fingerprint'));
  ...
  console.log(JSON.stringify({ok:true,version:'5.1.0',oauth_state:'pass',persistence:'pass',worker_recovery:'pass',orchestration:'pass',runtime_connectors:'pass',source_contracts:'pass',provenance:'pass',ui_actions:'pass',jarvis_ui_contract:'pass',base_connectors:93},null,2));
}
