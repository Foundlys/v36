...
async function fixture({workspaceId='sales',entity='opportunities',start=true,industry='GENERAL',shared,storage}={}){
 const f=workspaceFixture({workspaceId}),buckets=shared?.buckets||new Map(),ctx={tenant_id:'domain_ui_fixture',dealer_id:'default'},actor={id:'domain_fixture_owner',roles:['SUPER_ADMIN']},calls=[],overrides=new Map(),gates=[];
 const adapter={bucket(c,scope){const key=JSON.stringify([c,scope]);if(!buckets.has(key))buckets.set(key,[]);return buckets.get(key);},persist(){},audit(){},publish(){}},resolver=new CapabilityResolver(adapter);if(!shared)resolver.configure(ctx,actor,{industry_id:industry,entitlements:[workspaceId],expected_revision:0});const core=new BusinessDomain(workspaceId,adapter,resolver),saved=storage||new Map();f.context.sessionStorage={getItem:key=>saved.get(key)||null,setItem:(key,value)=>saved.set(key,String(value)),removeItem:key=>saved.delete(key)};
 let lost=false,denied=false;f.context.crypto.randomUUID=()=>require('node:crypto').randomUUID();f.ui.state.activeSection=entity.toUpperCase();f.ui.state.workspace={id:workspaceId,short_label:'Literal domain',domain_required_fields:DEFINITIONS[workspaceId].required};f.context.FoundlyCalendarRecurrence=require('../calendar-recurrence');
 f.context.fetch=async(route,options={})=>{
  ...
  try{
   ...
   if(overrides.has(method+' '+route))value=clone(overrides.get(method+' '+route));
    ...
    if(url.pathname==='/api/'+workspaceId+'/schema')value={ok:true,module_id:workspaceId,request_context:{...ctx,actor_id:actor.id},entities:DEFINITIONS[workspaceId].entities,required_fields:DEFINITIONS[workspaceId].required,industry_fields:require('../industry-field-contract').fieldContract(resolver,ctx,actor,workspaceId)};
    else if(collection==='record-requests'&&parts[2]==='recover')value={ok:true,...core.recoverRequest(ctx,actor,decodeURIComponent(id),input)};
 ...
 if(start)await render();return {...f,calls,ctx,actor,adapter,resolver,core,buckets,saved,content,render,form,field,edit,overrides,hold,lose(){lost=true;},deny(){denied=true;},writes:()=>calls.filter(c=>['POST','PUT'].includes(c.method))};
}
