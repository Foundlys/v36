...
class CrmView extends View{
 ...
 reset(){for(const element of Object.values(this.elements||{}))element.value='';}
...
function fixture(){
 ...
 const records={items:[{id:'literal_record',name:'Literal private record',revision:1,status:'Literal status',updated_at:'2026-09-24T12:00:00Z'}]},pipelines={items:[{id:'literal_pipeline',name:'Literal private pipeline'}]},board={board:{pipeline:{id:'literal_pipeline'},stages:[{id:'literal_stage',name:'Literal private stage',deals:[{id:'literal_deal',title:'Literal private deal',revision:1,value:1}],value:1}]}},customer={customer:{subject:{id:'literal_record',name:'Literal private customer'},deals:[],tasks:[],appointments:[],documents:[],timeline:[],history:[],consents:[],custom_objects:[]}},automations={items:[{id:'literal_automation',name:'Literal private automation',trigger:{type:'Literal trigger'},actions:[],enabled:true}]},zero={answer:'Literal private ZERO answer',ui_commands:[]};
 const response=route=>route==='/api/crm/leads/literal_record'?{record:records.items[0]}:route==='/api/crm/automations'?{record:{id:'literal_created_automation',revision:1}}:route.startsWith('/api/crm/leads?')||route.startsWith('/api/crm/contacts?')?records:route.startsWith('/api/crm/pipelines?')?pipelines:route.endsWith('/board')?board:route.endsWith('/360')?customer:route.startsWith('/api/crm/automations?')?automations:route==='/api/zero/turn'?zero:route.startsWith('/api/crm/dashboard')?dashboard:route.startsWith('/api/crm/summary')?summary:route==='/api/crm/status'?status:{};
 ...
 Object.assign(f.context,{window:f.context,URLSearchParams,sessionStorage:{},FoundlyCrmObjects:{appendCustomer(){}},crypto:{randomUUID:()=> 'crm-dashboard-key-'+(++requestId)},setTimeout:()=>1,clearTimeout(){},setInterval:()=>1,clearInterval(){}});
 vm.runInContext(source.replace(/\ninit\(\);\s*$/,''),f.context);
 return {...f,i:f.context.FoundlyI18n,calls,errors,dashboard,summary,status,records,pipelines,board,customer,automations,zero,deny:()=>denied=true,allow:()=>denied=false,setEditing(value){vm.runInContext('state.editing='+Boolean(value),f.context);f.context.renderDashboard();},currentDashboard(){return JSON.parse(vm.runInContext('JSON.stringify(state.dashboard)',f.context));},hold(route){holdRoute=route;remaining=1;let release;held=new Promise(resolve=>release=resolve);release.started=new Promise(resolve=>started=resolve);return release;}};
}
