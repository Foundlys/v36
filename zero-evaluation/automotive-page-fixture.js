...
function fixture(){
 ...
 const tabs=Object.fromEntries(['why','market','economics','comparables','risks','source'].map(name=>{const button=new View('button');button.dataset.tab=name;return [name,button];})),queryAll=f.context.document.querySelectorAll;
 f.context.document.querySelectorAll=selector=>selector==='.detail-tabs button'?Object.values(tabs):queryAll(selector);
 ...
 return {...f,i:f.context.FoundlyI18n,calls,status,overview,search,today,analysis,tabs,respond:(route,value)=>responses.set(route,value),clearAnalyses(){vm.runInContext('app.analyses.clear()',f.context);},deny:()=>deny=true,allow:()=>deny=false,hold(route){holdRoute=route;remaining=1;let release;held=new Promise(resolve=>release=resolve);release.started=new Promise(resolve=>started=resolve);return release;},key(name){return Object.values(f.nodes).flatMap(n=>n.all()).find(n=>n.getAttribute('data-i18n')==='automotive.status.'+name);}};
}
