...
function fixture(){
 const f=browserFixture('en-GB'),calls=[],store=new Map();let key=0,deny=false,held=null,holdRoute='',remaining=0,started=()=>{};
 ...
 f.context.fetch=async(route,options)=>{calls.push({route,options});const code=deny?403:200,payload=JSON.stringify(deny?{error:'RAW_PRIVATE_ERROR',code:'access_denied'}:results(route));if(held&&route.startsWith(holdRoute)&&remaining-->0){started();await held;}return {ok:code===200,status:code,text:async()=>payload};};
 ...
 return {...f,i:f.context.FoundlyI18n,calls,status,overview,search,today,analysis,clearAnalyses(){vm.runInContext('app.analyses.clear()',f.context);},deny:()=>deny=true,allow:()=>deny=false,hold(route){holdRoute=route;remaining=1;let release;held=new Promise(resolve=>release=resolve);release.started=new Promise(resolve=>started=resolve);return release;},key(name){return Object.values(f.nodes).flatMap(n=>n.all()).find(n=>n.getAttribute('data-i18n')==='automotive.status.'+name);}};
}
