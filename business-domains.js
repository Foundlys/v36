...
const DEFINITIONS = Object.freeze({
  ...
  calendar: { legacy: 'agenda', primary: 'events', entities: ['events','calendars','availability'], required: { events: ['title','start_at','end_at','timezone'], calendars: ['name','timezone'], availability: ['title','start_at','end_at','timezone'] } },
...
const OWNED_FIELDS = new Set(['title','name','content','description','status','value_cents','cost_cents','currency','probability','supplier_id','opportunity_id','pipeline_id','stage_id','owner_id','start_at','end_at','timezone','participants','calendar_id','recurrence','thread_id','to','subject_id','purpose','legal_basis','related_refs','industry_fields','due_at','direction','consent_status']);
...
class BusinessDomain {
  ...
  validate(entity,input,previous={}){
    ...
    for(const field of ['value_cents','cost_cents'])if(next[field]!==undefined&&(!Number.isSafeInteger(next[field])||next[field]<0))fail('amount_invalid','Bedragen moeten niet-negatieve gehele centen zijn');
  ...
  list(ctx,actor,entity,query={}){
    this.scope(ctx,actor);const limit=Math.max(1,Math.min(250,Number(query.limit)||100)),offset=Math.max(0,Number(query.offset)||0),q=String(query.q||'').toLowerCase();
  ...
  get(ctx,actor,entity,id){this.scope(ctx,actor);const row=this.bucket(ctx,entity).find(row=>row.id===id&&this.visible(row,actor));if(!row)fail('record_not_found','Record niet gevonden',404);return clone(row);}
  ...
  save(ctx,actor,entity,input,options={}){
    this.scope(ctx,actor,'write');const rows=this.bucket(ctx,entity),prior=options.id?this.get(ctx,actor,entity,options.id):null;
  ...
  flush(ctx,actor){
    for(const row of this.adapter.bucket(ctx,`${this.id}:outbox`).filter(r=>r.status==='PENDING').slice(0,50)){
  ...
  approve(ctx,actor,entity,id,input){
    ...
    this.recordEvent(ctx,actor,entity,row,'approved',input);this.adapter.persist();this.flush(ctx,actor);
  ...
  summary(ctx,actor){
    this.scope(ctx,actor);const by_entity=Object.fromEntries(this.definition.entities.map(entity=>[entity,this.list(ctx,actor,entity,{limit:100})]));
    const opportunities=by_entity.opportunities?.items||[],currency_groups={};
    ...
    return {module_id:this.id,by_entity,currency_groups,aggregate_scope:'RETURNED_RECORDS_ONLY',observed_at:new Date().toISOString(),no_fake_data:true};
  }
}
