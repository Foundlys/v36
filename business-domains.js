...
const DEFINITIONS = Object.freeze({
  ...
  procurement: { legacy: 'inkoop', primary: 'opportunities', entities: ['opportunities','suppliers','rfqs','bids','approval_policies','awards','outcome_observations','economics_snapshots','quotes','orders','documents','tasks'], required: {economics_snapshots:[],outcome_observations:[],approval_policies:['name','currency','minimum_value_cents','approval_steps'],awards:['title'], rfqs:['title','currency','lines'],bids:['title','rfq_id','rfq_revision','supplier_id','currency','lines','evidence_reference'],suppliers: ['name'], opportunities: ['title'], quotes: ['title'], orders: ['title'], documents: ['name'], tasks: ['title'] } },
...
class BusinessDomain {
  ...
  snapshotReadable(ctx,actor,row,options={}){return require('./procurement-economics').readable(this,ctx,actor,row,options)&& require('./procurement-outcomes').readable(this,ctx,actor,row,options)&& require('./marketing-creative-history').readable(this,ctx,actor,row)&& require('./analysis-action-access').readable(this,ctx,actor,row,options)&& require('./analysis-definition-history').readable(this,ctx,actor,row)&& require('./sales-sequence-access').readable(this,ctx,actor,row)&& require('./sales-snapshot-access').snapshotReadable(this,ctx,actor,row)&&require('./marketing-creative-reviews').reviewReadable(this,ctx,actor,row)&&require('./communication-drafts').readable(this,ctx,actor,row)&&require('./communication-replies').readable(this,ctx,actor,row)&&require('./communication-templates').readable(this,ctx,actor,row);}
  ...
  mutate(ctx,callback){return scopedMutation(this.adapter,ctx,[...this.definition.entities.map(entity=>entity===this.definition.primary?this.definition.legacy:`${this.id}:${entity}`),`${this.id}:idempotency`,`${this.id}:outbox`,'platform:audit',...(this.id==='procurement'?[require('./procurement-outcomes').OPERATIONS]:[]),...(this.id==='marketing'?[require('./marketing-creative-history').OPERATIONS]:[]),...(this.id==='sales'?[require('./sales-pipeline').OPERATIONS]:[]),...(this.id==='analysis'?[require('./analysis-definition-versions').OPERATIONS,require('./analysis-actions').OPERATIONS]:[]),...(this.id==='communication'?['communication:draft_operations',require('./communication-attachments').SCOPE,require('./communication-inbox').SCOPE,require('./communication-send-reviews').SCOPE,require('./communication-submissions').SCOPE,require('./communication-submission-recovery').SCOPE,require('./communication-mailboxes').SCOPE,require('./communication-mailboxes').ITEMS,require('./communication-comments').SCOPE,require('./communication-edit-sessions').SCOPE,require('./communication-binary-attachments').SCOPE]:[])],callback);}
