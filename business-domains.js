...
class BusinessDomain {
  ...
  snapshotReadable(ctx,actor,row,options={}){return require('./analysis-action-access').readable(this,ctx,actor,row,options)&& require('./analysis-definition-history').readable(this,ctx,actor,row)&& require('./sales-sequence-access').readable(this,ctx,actor,row)&& require('./sales-snapshot-access').snapshotReadable(this,ctx,actor,row)&&require('./marketing-creative-reviews').reviewReadable(this,ctx,actor,row)&&require('./communication-drafts').readable(this,ctx,actor,row)&&require('./communication-replies').readable(this,ctx,actor,row)&&require('./communication-templates').readable(this,ctx,actor,row);}
  ...
  conflicts(ctx,actor,input,exclude){
    ...
    return {count,visible_ids:visible,private_details_redacted:true,external_calendar_revision:external.revision||null,external_calendar_coverage:external.coverage||'NOT_CONFIGURED'};
  }
  mutate(ctx,callback){return scopedMutation(this.adapter,ctx,[...this.definition.entities.map(entity=>entity===this.definition.primary?this.definition.legacy:`${this.id}:${entity}`),`${this.id}:idempotency`,`${this.id}:outbox`,'platform:audit',...(this.id==='sales'?[require('./sales-pipeline').OPERATIONS]:[]),...(this.id==='analysis'?[require('./analysis-definition-versions').OPERATIONS,require('./analysis-actions').OPERATIONS]:[]),...(this.id==='communication'?['communication:draft_operations',require('./communication-attachments').SCOPE,require('./communication-inbox').SCOPE,require('./communication-send-reviews').SCOPE,require('./communication-submissions').SCOPE,require('./communication-submission-recovery').SCOPE,require('./communication-mailboxes').SCOPE,require('./communication-mailboxes').ITEMS,require('./communication-comments').SCOPE,require('./communication-edit-sessions').SCOPE,require('./communication-binary-attachments').SCOPE]:[])],callback);}
  ...
  export(ctx,actor){
    ...
    if(this.id==='calendar'&&this.external)collections.external_calendar_observations=this.external.exportOwned(ctx,actor,collections.calendars);
