...
const DEFINITIONS = Object.freeze({
  ...
  calendar: { legacy: 'agenda', primary: 'events', entities: ['events','calendars','availability','reminders'], required: { events: ['title','start_at','end_at','timezone'], calendars: ['name','timezone'], availability: ['title','start_at','end_at','timezone'],reminders:['title','due_at'] } },
...
class BusinessDomain {
  ...
  validate(entity,input,previous={}){
    ...
    if(this.id==='calendar'&&entity==='reminders'){timestamp(next.due_at);next.delivery_state='INTERNAL_ONLY';}
  ...
  summary(ctx,actor){
    ...
    const opportunities=this.definition.entities.includes('opportunities')?this.bucket(ctx,'opportunities').filter(row=>!row.deleted_at&&row.status!=='ARCHIVED'&&this.visible(row,actor)):[],currency_groups={};
