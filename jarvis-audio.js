...
(function exposeJarvisAudio(root,factory){
  ...
  class FoundlyAudioScene{
    ...
    applyMix(){
      if(!this.ready)return;const now=this.context.currentTime,stateFactor=SOUNDTRACK_STATE[this.state]??SOUNDTRACK_STATE.STANDBY,ducked=this.userSpeaking||this.jarvisSpeaking,drive={ACTIVATING:1,THINKING:.72,SEARCHING:1,PLANNING:.78,EXECUTING:1.18,VERIFYING:.66,SUCCESS:.62,WARNING:.48,ERROR:.18,RECOVERING:.55}[this.state]??.28;
  ...
  return {DoubleClapGate,FoundlyClapDetector,FoundlyAudioScene,SOUNDTRACK_STATE,ENERGY_CYCLE_SECONDS};
});
