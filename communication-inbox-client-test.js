...
async function mount({write=true}={}){
 ...
 context.window=context;vm.createContext(context);const guard=source.indexOf('  function creativeWorkCanLeave(');assert.ok(guard>0);vm.runInContext(source.slice(guard,source.indexOf('\n',guard)),context);vm.runInContext(source.slice(source.indexOf('  function appendCommunicationDeliveryReport('),source.indexOf('  function appendCommunicationConversation(')),context);vm.runInContext(source.slice(source.indexOf('  function appendCommunicationMailOAuth('),source.indexOf('  async function renderCommunicationMailbox(')),context);vm.runInContext(source.slice(start,end),context);await context.renderCommunicationInbox(content);
