...
function allocationFromRecords(rfq,getBid,input,{partial=false}={}){
  ...
  const allocations=input.map(line=>{
    ...
    if(!bids.has(line.bid_id))bids.set(line.bid_id,getBid(line.bid_id));const bid=bids.get(line.bid_id);
    if(!bid||bid.rfq_id!==rfq.id||bid.rfq_revision!==rfq.revision||bid.currency!==rfq.currency||['CANCELLED','ARCHIVED'].includes(bid.status))fail('allocation_bid_changed','Gebruik een actuele bieding voor deze aanvraag en valuta',409);
  ...
  if(partial&&rfq.lines.some(item=>(allocated.get(item.item_id)||0)>item.quantity))fail('allocation_scope_exceeded','De verdeling overschrijdt de gevraagde aantallen');
  if(!partial&&rfq.lines.some(item=>allocated.get(item.item_id)!==item.quantity))fail('allocation_scope_incomplete','Verdeel de volledige gevraagde aantallen precies één keer; beoordeling geldt voor het gezamenlijke totaal');
  return {rfq_id:rfq.id,rfq_revision:rfq.revision,allocation_kind:partial?'ITEM_INCREMENTAL':'ITEM_SPLIT',allocations:allocations.map(({bid_id,item_id,quantity})=>({bid_id,item_id,quantity})),allocation_lines:allocations,value_cents:total,currency:rfq.currency,title:`Artikelverdeling: ${rfq.title}`};
}
module.exports={allocationProposal,allocationFromRecords};
