...
function reviewCreative(domain,ctx,actor,id,input,options={}){
  ...
  return command(domain,ctx,actor,'review:'+id,input,options,()=>{
    ...
    if(row.status==='APPROVED_INTERNAL'){const current=domain.bucket(ctx,'creatives').find(item=>item.id===row.creative_id);const previous=clone(current);current.status='APPROVED_INTERNAL';current.revision++;current.updated_at=now;current.approved_by=actor.id;current.approval_reference_id=row.id;row.approved_creative_revision=current.revision;require('./marketing-creative-history').append(domain,ctx,actor,current,previous);domain.recordEvent(ctx,actor,'creatives',current,'approved');}
