...
const salesTotals=sales.summary(ctx,actor),usdSummary=salesTotals.currency_groups.USD;assert.equal(usdSummary.open_known_cents,10500,'All 105 authorized USD rows contribute beyond list pagination');assert.equal(usdSummary.weighted_cents,5250);assert.equal(usdSummary.open_cents,null,'Earlier opportunities without currency prevent a complete per-currency total');assert.equal(usdSummary.weighted_total_cents,null);assert.ok(salesTotals.sales_summary.unassigned_currency_count>0);assert.equal(usdSummary.source_coverage,'PARTIAL_SOURCE_RECORDS');
