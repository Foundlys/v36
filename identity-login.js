...
(()=>{
  ...
  for(const input of [username,password]){input.addEventListener('invalid',()=>{const key=input.validity.tooShort?'identity.password_invalid':'common.required';validationKeys.set(input,key);input.setCustomValidity(i18n.t(key));});input.addEventListener('input',()=>{validationKeys.delete(input);input.setCustomValidity('');});}
  document.addEventListener('foundly:locale',()=>{for(const [input,key]of validationKeys)input.setCustomValidity(i18n.t(key));});
  async function request(path,payload,method='POST'){
    const localizedError=key=>Object.assign(new Error(i18n.t(key)),{i18nKey:key});
    let response;try{response=await fetch(path,{method,credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});}catch{throw localizedError('common.offline');}
    let result;try{result=await response.json();}catch{throw localizedError('common.request_failed');}if(!response.ok)throw localizedError(i18n.errorKey(result.code,response.status));return result;
  }
  ...
  }catch(error){label('identityNotice',error.i18nKey||'common.request_failed');}finally{button.disabled=false;}});
})();
