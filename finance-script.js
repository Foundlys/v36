...
const financeValue=value=>financeCopy('value','{value}',{value}),financeAmount=(value,currency='EUR')=>financeValue(financeLive(()=>money(value,currency))),financeNumber=(value,positive=false)=>financeLive(()=>Number.isSafeInteger(value)&&value>=(positive?1:0)?new Intl.NumberFormat(globalThis.FoundlyI18n?.locale||'nl-NL').format(value):String(financeUnknown()));
...
function renderKpis(){
 ...
 const widgets=Array.isArray(state.dashboard?.widgets)?state.dashboard.widgets.filter(widget=>Object.hasOwn(MONEY_LABELS,widget.id)):[];
 for(const widget of widgets){const card=financeElement('article','','kpi-card'),valid=widget.available===undefined||widget.available===true;card.append(financeElement('h2',financeCopy('kpi.'+widget.id,MONEY_LABELS[widget.id])),financeElement('strong',valid?financeAmount(widget.value_cents):financeUnknown(),'kpi-value'),financeElement('span',state.dashboard.source==='posted_immutable_journal_entries'?financeCopy('posted_source','Geboekte, onveranderlijke journaalposten'):state.dashboard.source||financeUnknown(),'kpi-meta'));host.append(card);}
...
function renderAging(){
 ...
 financeRender($('#financeAgingBadge'),overdue===null?financeUnknown():overdue?financeCopy('overdue','{amount} ACHTERSTALLIG',{amount:financeLive(()=>money(overdue))}):financeCopy('no_overdue','GEEN ACHTERSTAND'));
...
function renderForecast(){
 ...
 if(budget?.available===true)rows.push(['budget_approved','Goedgekeurd budget',financeAmount(budget.budget_cents)],['actual','Werkelijk',financeAmount(budget.actual_cents)],['variance','Verschil',financeAmount(budget.variance_cents)]);
