...
function execute(drafts,platform,ctx,actor,action,{message,conversation_id,turn_id,prior=null}){
 ...
 const reference={operation:action.operation,tool_id:op.tool,...(action.operation==='INSPECT_RUN'?{run_id:action.run_id}:{draft_id:action.draft_id}),request_hash};
 if(action.operation==='INSPECT_RUN'){
