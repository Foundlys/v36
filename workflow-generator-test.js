...
const sandbox={sessionStorage:{getItem:key=>clientStorage.get(key)??null,setItem:(key,value)=>clientStorage.set(key,value),removeItem:key=>clientStorage.delete(key)},document:{createElement:tag=>new Element(tag)},crypto,FoundlyWorkflowAuthoring:require('./workflow-authoring')};vm.createContext(sandbox);vm.runInContext(fs.readFileSync(require.resolve('./workflow-generator.js'),'utf8'),sandbox);
...
const create=()=>sandbox.FoundlyWorkflowGenerator.create({document:sandbox.document,spec,zeroRequest,requestContext:{...ctx,actor_id:actor.id},recoverRequest:async(route,options)=>drafts.recoverNew(ctx,actor,route.split('/').at(-2),JSON.parse(options.body)),onSaved:r=>{record=r;}});
(async()=>{
