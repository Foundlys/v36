...
function compose(plan,{attempt_id,created_at,files=[]}){
 ...
 const snapshot=plan.snapshot;require('./communication-drafts').validateInput(snapshot);text(snapshot.content,12000);if(!Array.isArray(snapshot.attachments)||snapshot.attachments.length>10||files.length!==snapshot.attachments.length)fail();
