...
function outcome(core,ctx,row,entity,baseline){
 ...
 const unavailable=(reason_code,reason)=>({available:false,reason_code,reason,provider_verified:false,coverage:'RECORDED_LEDGER_ONLY'});
 if(end<=start)return unavailable('WINDOW_NOT_STARTED','Het prognosevenster is nog niet begonnen');
 const accounts=core.collection(ctx,'accounts').filter(a=>a.legal_entity_id===entity.id&&a.system_role==='BANK');
 if(!accounts.length)return unavailable('NO_CASH_ACCOUNTS','Geen geboekte kasrekeningen beschikbaar');
 if(accounts.some(a=>a.currency!==entity.currency||a.type!=='ASSET')||new Set(accounts.map(a=>a.id)).size!==accounts.length)return unavailable('AMBIGUOUS_CASH_ACCOUNTS','Kasrekeningmapping of valuta is niet eenduidig');
 ...
 if(ids.size!==entries.length||entries.some(e=>!['POSTED','REVERSAL','CORRECTION'].includes(e.status)||e.currency!==entity.currency))return unavailable('AMBIGUOUS_LEDGER','De geboekte bron is niet eenduidig in deze valuta');
 ...
 if(entries.length>50000||lines.length>250000)return unavailable('SOURCE_CAPACITY','De volledige bron overschrijdt de controlelimiet');
 ...
 if(new Set(lines.map(l=>l.id)).size!==lines.length)return unavailable('DUPLICATE_LINES','Dubbele boekingsregels');
 for(const entry of entries){const own=byJournal.get(entry.id)||[];let debit=0n,credit=0n;for(const l of own){if(!Number.isSafeInteger(l.debit_cents)||!Number.isSafeInteger(l.credit_cents)||l.debit_cents<0||l.credit_cents<0)return unavailable('INVALID_AMOUNTS','Ongeldige boekingsbedragen');debit+=BigInt(l.debit_cents);credit+=BigInt(l.credit_cents);}if(own.length<2||debit!==credit)return unavailable('UNBALANCED_LEDGER','De boekingsbron is onvolledig of niet in balans');}
 if(!cashLines.length)return unavailable('NO_RECORDED_MOVEMENTS','Geen vastgelegde kasbewegingen in het verstreken venster; ontbrekende registratie is geen nulresultaat');
