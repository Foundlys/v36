...
const PORT=20020+Math.floor(Math.random()*80),dataDir=fs.mkdtempSync('/tmp/foundly-oauth-production-'),base=`http://127.0.0.1:${PORT}`;
...
const env={...process.env,NODE_ENV:'production',NODE_OPTIONS:`--require=${preload}`,PORT:String(PORT),FOUNDLY_DATA_DIR:dataDir,FOUNDLY_PUBLIC_BASE_URL:base,FOUNDLY_ADMIN_USERNAME:'foundly',FOUNDLY_ADMIN_PASSWORD:password,FOUNDLY_ENCRYPTION_KEY:'oauth-regression-encryption-key',FOUNDLY_TENANT_ID:'tenant-regression',FOUNDLY_DEALER_ID:'dealer-regression',FOUNDLY_WORKER_INTERVAL_MS:'99999999',OPENAI_API_KEY:'',FOUNDLY_AI_API_KEY:'',META_APP_ID:'meta-client',META_APP_SECRET:'meta-secret',META_REDIRECT_URI:`${base}/api/connect/meta/callback`,LINKEDIN_CLIENT_ID:'linkedin-client',LINKEDIN_CLIENT_SECRET:'linkedin-secret',LINKEDIN_REDIRECT_URI:`${base}/api/connect/linkedin/callback`,TIKTOK_CLIENT_KEY:'tiktok-client',TIKTOK_CLIENT_SECRET:'tiktok-secret',TIKTOK_REDIRECT_URI:`${base}/api/connect/tiktok/callback`,GOOGLE_CLIENT_ID:'google-client',GOOGLE_CLIENT_SECRET:'google-secret',GOOGLE_REDIRECT_URI:`${base}/api/google/oauth/callback`,GOOGLE_ADS_DEVELOPER_TOKEN:'google-developer-token',WIX_APP_ID:'wix-app',WIX_APP_SECRET:'wix-secret',WIX_SHARE_URL_ID:'wix-share-id',WIX_REDIRECT_URI:`${base}/api/connect/wix/callback`};
...
(async()=>{try{
  ...
  const persistedFiles=fs.readdirSync(dataDir).filter(f=>/^(connector-|google-)/.test(f));assert(persistedFiles.length>=4);for(const file of persistedFiles){const raw=fs.readFileSync(path.join(dataDir,file),'utf8');assert(raw.includes('"encrypted": true'));assert(!raw.includes('mock-'))}
  console.log(JSON.stringify({ok:true,basic_auth_start:'pass',public_callbacks:'pass',providers:[...providers.map(x=>x.id),'wix'],wix_signed_instance:'pass',wix_native_install:'pass',restart_persistence:'pass',tenant_binding:'pass',state_integrity:'pass',one_time_replay:'pass',transient_retry:'pass',encrypted_tokens:'pass',bootstrap_and_connected:'pass'},null,2));
