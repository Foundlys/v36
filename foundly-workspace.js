...
(() => {
  ...
  function appendMessageDraftActions(record,cell,content){
    ...
    load.addEventListener('click',async()=>{
      ...
      try{
        ...
        if(preview.mode==='REPLY'&&preview.threading?.available===false)form.append(node('p','','Gespreksheaders zijn niet beschikbaar. Dit wordt bij de verzendbeoordeling opnieuw getoond.'));
  ...
  async function renderCommunicationInbox(content){
    ...
    replaceChildren(content,[node('h3','','Bewaarde berichten'),form,notice,results,mailbox]);
