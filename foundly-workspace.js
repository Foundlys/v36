...
(() => {
  ...
  async function renderCommunicationMailbox(panel,alive,onRefresh){
    ...
    panel.setAttribute('data-communication-mailbox','true');status.setAttribute('role','status');status.setAttribute('aria-live','polite');replaceChildren(panel,[node('h4','',owned('heading')),status,controls]);
    panel.canLeave=()=>{if(dirty||busy||pending){writeText(status,owned(pending?'pending':'unsaved'));return false;}return true;};state.creativeHistoryViews=[...(state.creativeHistoryViews||[]),panel];
    const load=async(retainedReason=null)=>{
      ...
      try{
        ...
        model.retained_mailboxes.forEach(validateMailbox);need(new Set(model.retained_mailboxes.map(m=>m.id)).size===model.retained_mailboxes.length);realm=model.request_context;
        ...
        const form=node('form'),label=node('label'),reason=node('input'),confirmLabel=node('label'),confirm=node('input'),button=node('button','secondary-button',owned(observed?.next_offset>0?'next':'sync')),clear=node('button','secondary-button',owned('clear'));reason.required=true;reason.maxLength=1000;reason.setAttribute('data-communication-mailbox-field','reason');confirm.type='checkbox';confirm.required=true;confirm.setAttribute('data-communication-mailbox-field','confirm');button.type='submit';clear.type='button';button.setAttribute('data-communication-mailbox-action','sync');clear.setAttribute('data-communication-mailbox-action','clear');label.append(node('span','',owned('reason')),reason);confirmLabel.append(confirm,node('span','',owned('confirm')));form.append(label,confirmLabel,button,clear);controls.append(form);if(retainedReason!==null){reason.value=retainedReason;confirm.checked=false;dirty=Boolean(retainedReason);}
        const restore=()=>{if(pending){reason.value=pending.reason;confirm.checked=true;}};form.addEventListener('input',()=>{if(pending)restore();else dirty=Boolean(reason.value||confirm.checked);});form.addEventListener('change',()=>{if(pending)restore();else dirty=Boolean(reason.value||confirm.checked);});clear.addEventListener('click',()=>{if(!current()||pending||busy)return;reason.value='';confirm.checked=false;dirty=false;writeText(status,owned('cleared'));});
        form.addEventListener('submit',async event=>{
          event.preventDefault();if(!current()||busy||!pending&&(!confirm.checked||!reason.value.trim()))return;
          if(!pending){const input={expected_revision:observed?.revision||0,confirm:true,reason:reason.value.trim(),account_binding:model.account_binding};pending={key:crypto.randomUUID(),input,body:JSON.stringify(input),reason:reason.value,fingerprint:null};}const sent=pending;restore();busy=true;[button,reason,confirm,clear].forEach(n=>n.disabled=true);writeText(status,owned('syncing'));
          try{
            if(!sent.fingerprint)sent.fingerprint=await digest({account_key:model.account_binding,input:sent.input});if(!current()||pending!==sent)return;
            const result=await request('/api/communication/mailboxes/sync',{method:'POST',headers:{'idempotency-key':sent.key},body:sent.body},current);if(!current()||pending!==sent)return;need(result.ok===true&&same(result.request_context,realm)&&result.account_binding===model.account_binding&&typeof result.deduplicated==='boolean'&&result.provider_mutated===false&&result.request_ack?.request_id===sent.key&&result.request_ack.request_fingerprint===sent.fingerprint);validateMailbox(result.mailbox);need(result.mailbox.id===result.request_ack.mailbox_id&&(!observed||result.mailbox.id===observed.id)&&result.mailbox.revision>sent.input.expected_revision);if(result.mailbox.status==='READING')throw Object.assign(Error('mailbox_read_pending'),{mailboxReading:true});pending=null;busy=false;dirty=false;if(result.mailbox.status==='UNAVAILABLE'){await load(sent.reason);return;}await onRefresh();if(current())await load();
          }catch(error){if(current()&&pending===sent){writeText(status,error?.mailboxInvalid?owned('invalid'):error?.mailboxReading?owned('reading'):owned('pending'));button.disabled=false;writeText(button,owned('retry'));}}
