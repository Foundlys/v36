...
(() => {
  ...
  function appendMessageDraftActions(record,cell,content,options={}){
    ...
    load.addEventListener('click',async()=>{
      ...
      try{
        const preview=await call(`/api/communication/messages/${encodeURIComponent(record.id)}/draft-preview?mode=${encodeURIComponent(mode.value)}`),form=node('form'),title=node('input'),to=node('input'),text=node('textarea'),save=node('button','primary-button','Concept opslaan');
        if(!alive())return;title.value=preview.title;title.required=true;title.maxLength=1000;to.value=(preview.suggested_to||[]).join(', ');to.required=true;to.maxLength=12000;to.readOnly=preview.mode==='REPLY';text.required=true;text.maxLength=12000;save.type='submit';save.disabled=!preview.recipient_available;
  ...
  function appendZeroCommunication(record,cell,content,kind){
    const button=node('button','secondary-button',kind==='draft'?'Met ZERO beoordelen':'Met ZERO beantwoorden');button.type='button';cell.append(button);
  ...
  function appendDraftEditor(record,cell,content){
    ...
    for(const [name,title,tag] of [['title','Titel','input'],['content','Inhoud','textarea'],['description','Omschrijving','textarea'],['to','Ontvangers, gescheiden door komma’s','input']]){const label=node('label','',title),input=node(tag);input.maxLength=name==='title'?1000:12000;input.required=['title','content'].includes(name);label.append(input);editor.append(label);fields.set(name,input);input.addEventListener('input',()=>{dirty=true;});}
    ...
    function values(){return Object.fromEntries([...fields].map(([name,input])=>[name,name==='to'?input.value.split(',').map(value=>value.trim()).filter(Boolean):input.value]));}
