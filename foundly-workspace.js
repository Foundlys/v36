...
(() => {
  ...
  async function renderDomainSection(entity, content) {
    ...
    try {
      ...
      const fieldNames = [...new Set([...required, 'description','status',...(state.workspaceId==='communication'&&entity==='drafts'?['to','cc']:[]),...(state.workspaceId==='sales'&&entity==='opportunities'?['expected_close_date','closed_date','forecast_category']:[]),...(state.workspaceId==='procurement'&&entity==='orders'?['evidence_reference']:[]),...(state.workspaceId==='procurement'&&entity==='approval_policies'?['allow_self_approval']:[]), ...(state.workspaceId==='calendar'&&['availability','events'].includes(entity)?['calendar_id','participants']:[]), ...(['procurement','sales'].includes(state.workspaceId) && ['opportunities','quotes','orders'].includes(entity) ? ['value_cents','currency','probability'] : [])])];
