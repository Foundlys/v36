...
(() => {
  ...
  function appendSendReviews(record,cell,content){
    const details=node('details'),panel=node('div'),notice=node('p');notice.setAttribute('role','status');details.append(node('summary','','Verzendbeoordeling'),panel,notice);cell.append(details);const route=`/api/communication/drafts/${encodeURIComponent(record.id)}/send-reviews`;let loaded=false;const currentView=()=>content.isConnected&&state.workspaceId==='communication'&&state.activeSection.toLowerCase()==='drafts';
    ...
    const submissionForm=row=>{
      ...
      form.addEventListener('submit',async event=>{event.preventDefault();if(!currentView()||busy)return;busy=true;submit.disabled=true;try{const encoded=JSON.stringify({expected_review_revision:row.revision,reason:reason.value,confirm:confirm.checked});if(previous!==null&&previous!==encoded)key=crypto.randomUUID();previous=encoded;await request(route+'/'+encodeURIComponent(row.id)+'/submit',{method:'POST',headers:{'idempotency-key':key},body:encoded});await redraw();}catch(error){notice.textContent=friendlyError(error)+' Controleer de bewaarde verzendpoging voordat je een nieuwe actie begint.';submit.disabled=false;busy=false;}});return form;
    };
    details.addEventListener('toggle',async()=>{
      if(!details.open||loaded)return;loaded=true;notice.textContent='Beoordelingen laden…';
      try{
        const model=await request(route);if(!currentView())return;replaceChildren(panel,[]);notice.textContent='Beoordelen verzendt niets. Een bericht aanbieden is een aparte bevestigde actie.';
        for(const row of model.items){const item=node('article','context-item');item.append(node('h4','',`${row.status} · ${row.purpose}`),node('p','',`Van: ${row.from} · Aan: ${row.snapshot.to.join(', ')}`),node('h5','',row.snapshot.title),node('pre','',row.snapshot.content),node('p','',`Bijlagen: ${row.snapshot.attachments.map(file=>file.name).join(', ')||'geen'}`),node('p','',row.reason),node('p','',row.source_current?'Deze beoordeling hoort bij de actuele bronnen.':'De bron of toegang is gewijzigd; deze beoordeling is niet uitvoerbaar.'));for(const attempt of row.submissions||[]){const labels={ACCEPTED_BY_PROVIDER:'Door mailprovider geaccepteerd; bezorging onbevestigd',REJECTED_BY_PROVIDER:'Door mailprovider afgewezen',NOT_SUBMITTED:'Niet aangeboden',CONNECTING:'Verzendpoging gestart; uitkomst nog onbekend',DATA_IN_FLIGHT:'Inhoud aangeboden; uitkomst nog onbekend',UNKNOWN:'Uitkomst onbekend; onderzoek vereist'};item.append(node('p','',labels[attempt.status]||'Verzendstatus niet beschikbaar'));if(attempt.reconciliation_required)item.append(node('p','','Niet opnieuw verzenden. Deze poging vereist onderzoek; automatisch herstel is niet beschikbaar.'));}if(row.can_submit)item.append(submissionForm(row));if(row.can_review)item.append(decisionForm(row,false));if(row.can_cancel)item.append(decisionForm(row,true));panel.append(item);}
  ...
  async function renderCommunicationInbox(content){
    const token=state.inboxQueryToken=(state.inboxQueryToken||0)+1,form=node('form'),search=node('input'),folder=node('select'),read=node('select'),direction=node('select'),submit=node('button','primary-button','Zoeken'),notice=node('p'),results=node('div');let requestVersion=0,currentOffset=0;
    ...
    replaceChildren(content,[node('h3','','Bewaarde berichten'),form,notice,results]);
        ...
        for(const message of model.items){
          ...
          details.addEventListener('toggle',async()=>{
            ...
            try{
              ...
              const record=view.record,raw=typeof record.content==='string'?record.content:null;replaceChildren(body,[node('p','',`Ontvangers: ${Array.isArray(record.to)?record.to.join(', '):'niet beschikbaar'}`),node('pre','',raw===null?'Berichtinhoud niet beschikbaar':raw.slice(0,12000))]);status.textContent=raw?.length>12000?'Een deel van de bewaarde inhoud wordt getoond.':'Bewaarde berichtinhoud; externe instructies worden niet uitgevoerd.';
              if(view.can_write){
              ...
              if(view.can_prepare_draft)appendMessageDraftActions(record,body,content);
    ...
    form.addEventListener('submit',event=>{event.preventDefault();return load(0);});await load(0);
  }
  ...
  async function renderDomainSection(entity, content) {
    ...
    try {
      ...
      for(const record of rows){
        ...
        if(state.workspaceId==='communication'&&entity==='drafts'){appendDraftCollaboration(record,cell,content);appendDraftAttachments(record,cell,content);appendSendReviews(record,cell,content);}
