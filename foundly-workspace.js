  ...
  async function renderAutomationSection(section,content,query={}) {
    ...
    try {
      ...
      if(section==='WORKFLOWS'&&data.can_manage){
        ...
        const rememberDraft=record=>{const index=drafts.items.findIndex(row=>row.id===record.id);if(index>=0)drafts.items[index]=record;else drafts.items.push(record);let option=[...chooser.options].find(row=>row.value===record.id);if(!option){option=node('option');option.value=record.id;chooser.append(option);}option.textContent=`${record.draft.name||'Naamloos concept'} · revisie ${record.revision}`;chooser.value=record.id;};
        const show=draft=>{const editor=window.FoundlyWorkflowEditor.create({document,spec:data.editor_contract,request,draft,onDraftRecovered:rememberDraft,requestContext:drafts.request_context,isActive:()=>content.isConnected&&state.workspaceId==='automation'&&state.activeSection===section&&token===state.automationQueryToken,zeroRequest:async(action,turnId)=>{if(!editor.isConnected||!content.isConnected||state.workspaceId!=='automation'||state.activeSection!==section||token!==state.automationQueryToken)throw Error('De workflowweergave is niet meer actief.');const response=await request('/api/zero/turn',{method:'POST',body:JSON.stringify({message:action.operation==='SAVE'?'Bewaar het expliciet bevestigde workflowconcept':'Bereid het gekozen workflowconcept voor',conversation_id:state.conversationId,turn_id:turnId,preferred_module:'automation',client_context:{automation_action:action}})});if(!editor.isConnected||!content.isConnected||state.workspaceId!=='automation'||state.activeSection!==section||token!==state.automationQueryToken)throw Error('De workflowweergave is niet meer actief.');state.conversationId=response.conversation_id||state.conversationId;return response.automation_data;},onSaved:()=>renderAutomationSection(section,content)});editorBox.replaceChildren(editor);};show();
        ...
        const language=window.FoundlyWorkflowGenerator.create({document,spec:data.editor_contract,requestContext:drafts.request_context,recoverRequest:request,isActive:()=>content.isConnected&&state.workspaceId==='automation'&&state.activeSection===section,zeroRequest:async(action,turnId)=>{
