...
(() => {
    ...
    try {
      ...
      for(const row of rows||[]){
        ...
        if(section==='WORKFLOWS'&&data.can_manage&&row.can_activate){
          ...
          controls.addEventListener('submit',async event=>{event.preventDefault();if(!current()||activate.disabled)return;const active=event.submitter?.value==='activate';activate.disabled=pause.disabled=true;try{await read(`/api/automation/workflows/${row.id}/activation`,{method:'PUT',body:JSON.stringify({active,confirm:confirmation.checked,expected_revision:row.activation_revision,reason:reason.value.trim()})});if(current())await renderAutomationSection(section,content);}catch(error){if(current())notice.textContent=friendlyError(error);if(current()){activate.disabled=false;pause.disabled=row.activation_mode==='PAUSED';}}});
        }
        if(section==='WORKFLOWS'&&data.can_manage)appendManualWorkflowRun(row,card,content,current);
        if(section==='APPROVALS'&&data.can_approve){
          ...
          form.addEventListener('submit',async event=>{event.preventDefault();if(!current()||button.disabled)return;button.disabled=true;try{await read(`/api/automation/workflows/${row.automation_id}/runs`,{method:'POST',body:JSON.stringify({event:row.trigger,options:{inputs:row.inputs,approval:{run_id:row.run_id,request_signature:row.request_signature,reference:crypto.randomUUID(),reason:input.value.trim()}}})});if(current())await renderAutomationSection(section,content);}catch(error){if(current())notice.textContent=friendlyError(error);if(current())button.disabled=false;}});card.append(form);
        }
        result.push(card);
      }
  ...
  function appendManualWorkflowRun(workflow,card,content,isCurrent=()=>true){
