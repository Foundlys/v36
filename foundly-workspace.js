...
(() => {
  ...
  async function renderProcurementComparison(record,content,notice,isActive=()=>true) {
    ...
    try {
      ...
      for(const bid of comparison.items){
        ...
        if(bid.comparable){const button=procurementButton(item,'prepare_bid');button.addEventListener('click',()=>{if(active()&&item.isConnected)return prepareProcurementAward(comparison,bid,item,undefined,'FULL','native',{isActive:active});});}panel.append(item);
      }
  ...
  async function renderDomainSection(entity, content) {
    ...
    try {
      ...
      for(const record of rows){
        ...
        if(workspace==='procurement'&&window.FoundlyProcurementIntelligence&&(['rfqs','suppliers'].includes(entity)||entity==='awards'&&record.status==='APPROVED_INTERNAL')){const detail=node('details');detail.setAttribute('data-procurement-intelligence',entity);detail.append(node('summary','',live(()=>i18n().t('procurement.intelligence.'+(entity==='rfqs'?'costs':entity==='suppliers'?'supplier_summary':'record_outcome')))));let loaded=false;detail.addEventListener('toggle',()=>{if(!detail.open||loaded)return;loaded=true;const active=()=>current(),view=window.FoundlyProcurementTransport.create({document,request,isActive:active,build:call=>window.FoundlyProcurementIntelligence.create({document,request:call,isActive:active,...(entity==='rfqs'?{rfqId:record.id}:entity==='suppliers'?{supplierId:record.id}:{award:record})})});state.creativeHistoryViews=[...(state.creativeHistoryViews||[]).filter(v=>v.isConnected),view];detail.append(view);});cell.append(detail);}
        ...
        if(workspace==='procurement'&&entity==='rfqs'){const compare=procurementButton(cell,'compare');compare.addEventListener('click',()=>renderProcurementComparison(record,content,notice,current));}
        if(workspace==='procurement'&&entity==='orders'&&!['APPROVED_INTERNAL','ARCHIVED','CANCELLED'].includes(record.status)){const prepare=procurementButton(cell,'prepare_order');prepare.addEventListener('click',()=>prepareProcurementAward({order_id:record.id,order_revision:record.revision,value_cents:record.value_cents,currency:record.currency},null,cell,undefined,'FULL','native',{isActive:current}));}
        if(workspace==='procurement'&&entity==='awards')appendAwardReview(record,cell,content,current);
