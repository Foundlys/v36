...
(() => {
  ...
  function appendMessageDraftActions(record,cell,content,options={}){
    ...
    const message=key=>writeText(notice,owned(key)),restore=()=>{for(const x of frozen)x.n.value=x.value;},reset=()=>{version++;plan=null;reading=false;controls=[];replaceChildren(previewBody,[]);load.disabled=false;mode.disabled=false;},protect=()=>{if(pending||busy){restore();message('pending');return true;}if(dirty){message('unsaved');return true;}return false;};
    ...
    load.addEventListener('click',async()=>{
      if(!alive()||protect()||reading)return;reading=true;load.disabled=true;const current=++version,kind=selectedMode;plan=null;
      try{
        ...
        form.addEventListener('submit',async event=>{
          event.preventDefault();if(!alive()||!form.isConnected||busy||plan!==preview||!preview.recipient_available||!text.value.trim()||!title.value.trim()||!to.value.trim())return;busy=true;save.disabled=true;load.disabled=true;mode.disabled=true;clear.disabled=true;
          ...
          try{
            const result=await call('/api/communication/messages/'+encodeURIComponent(record.id)+'/drafts',{method:'POST',headers:{'idempotency-key':attempt.key},body:attempt.body},()=>alive()&&pending===attempt);if(!alive()||pending!==attempt)return;const row=result.record,ref=row?.source_message_ref,input=JSON.parse(attempt.body),realm=options.requestContext;need(result.external_send===false&&typeof result.deduplicated==='boolean'&&row?.owned_entity==='drafts'&&row.tenant_id===realm?.tenant_id&&row.dealer_id===realm?.dealer_id&&row.owner_id===realm?.actor_id&&row.revision===1&&row.status==='DRAFT'&&row.delivery_state==='NOT_SENT'&&!row.deleted_at&&row.title===input.title&&row.content===input.content&&same(row.to,input.to)&&same(row.cc||[],input.cc||[])&&ref?.id===record.id&&ref.revision===input.source_revision&&ref.hash===input.source_hash&&ref.mode===kind&&(kind!=='REPLY_ALL'||ref.hash_version===2&&ref.reply_from===preview.reply_from));pending=null;dirty=false;plan=null;replaceChildren(previewBody,[node('p','',owned('saved_help'))]);message('saved');load.disabled=false;mode.disabled=false;clear.disabled=false;
          }catch(error){if(alive()&&pending===attempt){message('pending');save.disabled=false;}}finally{busy=false;}
        });
  ...
  async function zeroCommunicationRequest(source,route,options={},alive=()=>true){
    ...
    if(action.operation==='CREATE_TEMPLATE_DRAFT'&&options.headers?.['idempotency-key'])action.request_id=options.headers['idempotency-key'];
  ...
  async function renderCommunicationInbox(content){
    ...
    const load=async(offset=0)=>{
      ...
      try{
        ...
        if(!alive()||version!==requestVersion||!creativeWorkCanLeave())return;replaceChildren(results,[]);notice.textContent=`${model.total_retained_matching} passende bewaarde berichten. De volledigheid van de externe mailbox is onbekend. Leesstatus en archief gelden alleen voor jou in Foundly.`;
        ...
        for(const message of model.items){
          ...
          details.addEventListener('toggle',async()=>{
            ...
            try{
              ...
              if(view.can_prepare_draft){const options={alive:()=>alive()&&version===requestVersion,requestContext:view.request_context};appendMessageDraftActions(record,body,content,options);appendZeroCommunication(record,body,content,'message',options);}
