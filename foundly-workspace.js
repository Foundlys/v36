  ...
  async function renderAutomationSection(section,content,query={}) {
    ...
    try {
      ...
      if(section==='WORKFLOWS'&&data.can_manage){
        ...
        const language=window.FoundlyWorkflowGenerator.create({document,spec:data.editor_contract,isActive:()=>content.isConnected&&state.workspaceId==='automation'&&state.activeSection===section,zeroRequest:async(action,turnId)=>{
