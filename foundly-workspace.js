  ...
  async function renderDomainSection(entity, content) {
    ...
    try {
      ...
      for(const record of rows){
        ...
        if(!['messages','notifications','awards','forecast_snapshots','creative_reviews','creative_revisions','draft_revisions','economics_snapshots','outcome_observations','supplier_clarifications'].includes(entity)&&record.status!=='APPROVED_INTERNAL')cell.append(edit);tr.append(cell);body.append(tr);
      }
      const summary=node('p','',`${result.total} records${result.next_offset!==null?' · eerste 100 getoond':''}`);
      const children=[summary];if(!['messages','notifications','awards','forecast_snapshots','creative_reviews','creative_revisions','draft_revisions','economics_snapshots','outcome_observations','supplier_clarifications'].includes(entity))children.push(form);
