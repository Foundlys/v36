...
(() => {
  ...
  function appendSendReviews(record,cell,content){
    ...
    details.addEventListener('toggle',async()=>{
      ...
      try{
        const model=await request(route);if(!currentView())return;replaceChildren(panel,[]);notice.textContent='Dit is een interne beoordeling. Er wordt niets verzonden.';
        for(const row of model.items){const item=node('article','context-item');item.append(node('h4','',`${row.status} · ${row.purpose}`),node('p','',`Van: ${row.from} · Aan: ${row.snapshot.to.join(', ')}`),node('h5','',row.snapshot.title),node('pre','',row.snapshot.content),node('p','',`Bijlagen: ${row.snapshot.attachments.map(file=>file.name).join(', ')||'geen'}`),node('p','',row.reason),node('p','',row.source_current?'Deze beoordeling hoort bij de actuele bronnen.':'De bron of toegang is gewijzigd; deze beoordeling is niet uitvoerbaar.'));if(row.can_review)item.append(decisionForm(row,false));if(row.can_cancel)item.append(decisionForm(row,true));panel.append(item);}
