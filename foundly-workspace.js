...
(() => {
  ...
  function appendTemplateDraft(record,cell,content,options={}){
    ...
    const details=node('details'),panel=node('div'),notice=node('p'),message=(key,p)=>writeText(notice,owned(key,p));details.setAttribute('data-communication-template',record.id);notice.setAttribute('role','status');notice.setAttribute('aria-live','polite');details.append(node('summary','',owned('panel')),panel,notice);cell.append(details);let loaded=false,dirty=false,busy=false,pending=null;
    details.canLeave=()=>{if(!alive())return true;if(dirty||busy||pending){message(pending?'pending':'unsaved');return false;}return true;};state.creativeHistoryViews=[...(state.creativeHistoryViews||[]).filter(v=>v.isConnected),details];
    details.addEventListener('toggle',async()=>{
      ...
      try{
        ...
        clear.addEventListener('click',()=>{if(!alive())return;if(pending||busy){message('pending');return;}generation++;plan=null;dirty=false;for(const n of controls){n.value='';n.checked=false;}save.disabled=true;replaceChildren(previewBody,[]);message('cleared');});
        ...
        form.addEventListener('submit',async event=>{event.preventDefault();if(!alive()||busy||!plan||!confirm.checked||!reason.value.trim())return;busy=true;save.disabled=true;previewButton.disabled=true;clear.disabled=true;const selected=plan;if(!pending){pending={key:crypto.randomUUID(),body:JSON.stringify({...selected.input,source_revision:selected.source_revision,source_hash:selected.source_hash,preview_fingerprint:selected.preview_fingerprint,confirm:true,reason:reason.value})};freeze();}const attempt=pending;try{const result=await call(route+'/drafts',{method:'POST',headers:{'idempotency-key':attempt.key},body:attempt.body},()=>alive()&&pending===attempt);if(!alive()||pending!==attempt)return;const row=result.record,ref=row?.source_template_ref;need(result.external_send===false&&typeof result.deduplicated==='boolean'&&row?.owned_entity==='drafts'&&row.tenant_id===record.tenant_id&&row.dealer_id===record.dealer_id&&row.owner_id===options.requestContext?.actor_id&&row.tenant_id===options.requestContext?.tenant_id&&row.dealer_id===options.requestContext?.dealer_id&&row.revision===1&&row.status==='DRAFT'&&row.delivery_state==='NOT_SENT'&&!row.deleted_at&&row.title===selected.title&&row.content===selected.content&&same(row.to,selected.to)&&same(row.cc,selected.cc)&&ref?.id===record.id&&ref.revision===selected.source_revision&&ref.hash===selected.source_hash&&ref.values_hash===await digest(selected.input.variables));if(!alive()||pending!==attempt)return;pending=null;dirty=false;plan=null;replaceChildren(panel,[node('p','',owned('saved_help'))]);message('saved');}catch(error){if(alive()&&pending===attempt){message('pending');save.disabled=false;}}finally{busy=false;}});
  ...
  async function zeroCommunicationRequest(source,route,options={},alive=()=>true){
    ...
    if(!alive())throw Error('De gekozen Communication-bron is niet meer actief.');source.conversation_id=result.conversation_id;state.conversationId=result.conversation_id||state.conversationId;if(source.kind==='template')writeText(byId('zeroOutput'),live(()=>i18n().t('communication.template.zero_returned')));else byId('zeroOutput').textContent=result.display_text||result.answer;if(!result.communication_data)throw Error('De actuele Communication-uitkomst is niet beschikbaar.');return result.communication_data;
  }
  function appendZeroCommunication(record,cell,content,kind,contextOptions={}){
    const button=node('button','secondary-button',kind==='template'?live(()=>i18n().t('communication.template.zero_open')):kind==='draft'?'Met ZERO beoordelen':'Met ZERO beantwoorden');button.type='button';cell.append(button);button.setAttribute('data-communication-zero',kind);
  ...
  async function renderDomainSection(entity, content) {
    ...
    try {
      ...
      for(const record of rows){
        ...
        if(workspace==='communication'&&entity==='templates'){appendTemplateDraft(record,cell,content,{alive:current,requestContext:realm});appendZeroCommunication(record,cell,content,'template',{alive:current,requestContext:realm});}
