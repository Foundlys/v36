...
(() => {
  ...
  function appendForecastSnapshotForm(parent,result=null,filters=null,scenario=null,hierarchy=null) {
    ...
    const view=window.FoundlyForecastSnapshot.create({document,request:(path,options)=>request(path,options,current),result,filters,scenario,hierarchy,isActive:current});parent.append(view);state.creativeHistoryViews=[...(state.creativeHistoryViews||[]).filter(v=>v.isConnected),view];return view;
  }
