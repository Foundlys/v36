...
(() => {
  ...
  function appendSendReviews(record,cell,content,options={}){
    ...
    details.addEventListener('toggle',async()=>{
      ...
      }catch(error){if(error.reviewContextInvalid){replaceChildren(panel,[]);writeText(notice,live(()=>i18n().t('communication.review_context.invalid')));}else notice.textContent=friendlyError(error);loaded=false;}
    });
  }
  ...
  function appendCommunicationConversation(record,body,alive,requestContext){
    const owned=(key,p={})=>live(()=>i18n().t('communication.conversation.'+key,typeof p==='function'?p():p)),details=node('details'),panel=node('div'),status=node('p'),realm=JSON.parse(JSON.stringify(requestContext)),active=()=>alive()&&details.isConnected&&details.open;let loaded=false,version=0,busy=false;
    const need=value=>{if(!value)throw Object.assign(Error('communication_conversation_invalid'),{conversationInvalid:true});},object=v=>v!==null&&typeof v==='object'&&!Array.isArray(v),id=v=>typeof v==='string'&&/^[A-Za-z0-9_.:-]{1,200}$/.test(v),integer=(v,min=0)=>Number.isSafeInteger(v)&&v>=min,context=v=>object(v)&&Object.keys(v).length===3&&['tenant_id','dealer_id','actor_id'].every(k=>id(v[k])),same=(a,b)=>JSON.stringify(a)===JSON.stringify(b),nullable=(v,max)=>v===null||typeof v==='string'&&v.length<=max,stamp=v=>typeof v==='string'&&Number.isFinite(Date.parse(v)),addresses=v=>v===null||Array.isArray(v)&&v.length<=100&&v.every(v=>typeof v==='string'&&v.length<=320),failure=e=>e.conversationInvalid?owned('invalid'):live(()=>friendlyError(e));
    const validContext=v=>context(realm)&&context(v)&&['tenant_id','dealer_id','actor_id'].every(k=>v[k]===realm[k]);
    function validate(model,offset){need(object(model)&&model.ok===true&&validContext(model.request_context)&&model.source_id===record.id&&model.source_revision===record.revision&&integer(model.source_revision,1)&&integer(model.total_retained_visible,1)&&model.total_retained_visible<=25000&&model.limit===25&&model.offset===offset&&model.next_offset===(offset+25<model.total_retained_visible?offset+25:null)&&Array.isArray(model.items)&&model.items.length===Math.max(0,Math.min(25,model.total_retained_visible-offset))&&new Set(model.items.map(r=>r?.id)).size===model.items.length&&model.items.every(r=>object(r)&&id(r.id)&&integer(r.revision,1)&&nullable(r.title,1000)&&nullable(r.from,320)&&addresses(r.to)&&addresses(r.cc)&&[null,'INBOUND','OUTBOUND'].includes(r.direction)&&[r.sent_at,r.received_at,r.authored_at].every(v=>v===null||stamp(v))&&typeof r.content_available==='boolean')&&Array.isArray(model.participants)&&model.participants.length<=2000&&model.participants.every(v=>typeof v==='string'&&v.length<=320&&/^[^\s@]+@[^\s@]+$/.test(v))&&new Set(model.participants).size===model.participants.length&&['participants_complete','participants_window_limited','ambiguous_identifiers','headers_available'].every(k=>typeof model[k]==='boolean')&&(!model.participants_window_limited||model.participants_complete===false&&model.participants.length===2000)&&model.participant_identity_verified===false&&model.linkage==='UNVERIFIED_MESSAGE_HEADERS'&&model.external_thread_total===null&&model.external_thread_complete===false&&typeof model.snapshot_sha256==='string'&&/^[a-f0-9]{64}$/.test(model.snapshot_sha256)&&stamp(model.observed_at)&&Date.parse(model.observed_at)<=Date.now());}
    details.setAttribute('data-communication-conversation','true');status.setAttribute('role','status');status.setAttribute('aria-live','polite');details.append(node('summary','',owned('heading')),status,panel);body.append(details);
    const load=async(offset=0)=>{if(!active()||busy)return;const ticket=++version,current=()=>active()&&ticket===version;busy=true;for(const b of panel.querySelectorAll('button'))b.disabled=true;writeText(status,owned('loading'));try{
      const result=await request(`/api/communication/messages/${encodeURIComponent(record.id)}/conversation?limit=25&offset=${offset}`,{},current);if(!current())return;validate(result,offset);replaceChildren(panel,[]);loaded=true;writeText(status,owned('coverage',()=>({count:i18n().number(result.total_retained_visible)})));if(result.ambiguous_identifiers)panel.append(node('p','',owned('ambiguous')));if(!result.headers_available)panel.append(node('p','',owned('headers_unavailable')));
      panel.append(node('p','',owned('participants',()=>({addresses:result.participants.join(', ')||i18n().t('communication.conversation.unavailable')}))));if(!result.participants_complete)panel.append(node('p','',owned('participants_incomplete')));if(result.participants_window_limited)panel.append(node('p','',owned('participants_limited')));
      for(const message of result.items){const item=node('details'),content=node('div'),notice=node('p'),time=message.sent_at||message.received_at||message.authored_at;let opened=false,detailVersion=0;const visible=()=>current()&&item.isConnected&&item.open;item.setAttribute('data-communication-conversation-message',message.id);notice.setAttribute('role','status');notice.setAttribute('aria-live','polite');item.append(node('summary','',message.title||live(()=>i18n().t('communication.inbox.subject_unavailable'))),node('p','',owned('metadata',()=>({from:message.from||i18n().t('communication.inbox.from_unknown'),time:time?i18n().date(time,{dateStyle:'medium',timeStyle:'short',timeZone:'UTC'}):i18n().t('communication.inbox.time_unknown')}))),content,notice);panel.append(item);
        item.addEventListener('toggle',async()=>{if(!item.open){detailVersion++;opened=false;return;}if(!visible()||opened)return;const detailTicket=++detailVersion,currentView=()=>visible()&&detailTicket===detailVersion;opened=true;writeText(notice,owned('loading_message'));try{const view=await request('/api/communication/messages/'+encodeURIComponent(message.id)+'/view',{},currentView);if(!currentView())return;const row=view.record;need(view.ok===true&&validContext(view.request_context)&&row&&row.id===message.id&&row.revision===message.revision&&!row.deleted_at&&row.provider_currently_draft!==true&&row.status!=='ARCHIVED'&&(row.title??null)===message.title&&(row.from??null)===message.from&&same(row.to??null,message.to)&&same(row.cc??null,message.cc)&&(row.direction??null)===message.direction&&['sent_at','received_at','authored_at'].every(k=>(row[k]??null)===message[k])&&view.content_kind==='UNTRUSTED_RETAINED_MESSAGE'&&view.provider_updated===false);const raw=row.content;replaceChildren(content,[node('pre','',row.content_available!==false&&row.content_complete!==false&&typeof raw==='string'?raw:live(()=>i18n().t('communication.inbox.content_unknown')))]);writeText(notice,owned('content_retained'));}catch(error){if(currentView()){replaceChildren(content,[]);writeText(notice,failure(error));opened=false;}}});
      }
      for(const [key,next] of [['previous',offset>0?Math.max(0,offset-25):null],['next',result.next_offset]])if(next!==null){const button=node('button','secondary-button',owned(key));button.type='button';button.setAttribute('data-communication-conversation-action',key);button.addEventListener('click',()=>{if(button.isConnected&&!button.disabled)return load(next);});panel.append(button);}
    }catch(error){if(current()){replaceChildren(panel,[]);writeText(status,failure(error));loaded=false;}}finally{if(ticket===version)busy=false;}};
    details.addEventListener('toggle',()=>{if(!details.open){version++;busy=false;loaded=false;return;}if(!loaded)return load(0);});
  }
  function appendReplyContext(snapshot,panel){
    const reply=snapshot.reply_context;if(reply===undefined||reply===null)return;const owned=(key,p={})=>live(()=>i18n().t('communication.review_context.'+key,typeof p==='function'?p():p)),object=v=>v!==null&&typeof v==='object'&&!Array.isArray(v),header=v=>{if(typeof v!=='string'||v.length>254||!/^<[^<>\s]+@[^<>\s]+>$/.test(v))return false;const text=v.slice(1,-1),at=text.indexOf('@'),atom=/^[A-Za-z0-9!#$%&'*+\-/=?^_`{|}~]+(?:\.[A-Za-z0-9!#$%&'*+\-/=?^_`{|}~]+)*$/;return at===text.lastIndexOf('@')&&atom.test(text.slice(0,at))&&(atom.test(text.slice(at+1))||/^\[[\x21-\x5a\x5e-\x7e]+\]$/.test(text.slice(at+1)));},valid=value=>{if(!value)throw Object.assign(Error('communication_reply_context_invalid'),{reviewContextInvalid:true});};
    valid(object(reply)&&['REPLY','REPLY_ALL','FORWARD'].includes(reply.mode)&&typeof reply.source_id==='string'&&/^[A-Za-z0-9_.:-]{1,200}$/.test(reply.source_id)&&Number.isSafeInteger(reply.source_revision)&&reply.source_revision>=1&&typeof reply.source_hash==='string'&&/^[a-f0-9]{64}$/.test(reply.source_hash)&&reply.linkage==='RETAINED_SOURCE_REFERENCE'&&reply.provider_thread_verified===false&&object(reply.headers));const h=reply.headers;
    valid(typeof h.available==='boolean'&&Array.isArray(h.in_reply_to)&&h.in_reply_to.length<=1&&h.in_reply_to.every(header)&&Array.isArray(h.references)&&h.references.length<=100&&h.references.every(header)&&(h.available?h.reason===null:!h.in_reply_to.length&&!h.references.length&&['SOURCE_THREAD_HEADERS_UNAVAILABLE','REFERENCE_CHAIN_LIMIT'].includes(h.reason))&&(reply.mode==='FORWARD'?h.available&&!h.in_reply_to.length&&!h.references.length:!h.in_reply_to.length||h.references.at(-1)===h.in_reply_to[0]));
    panel.append(node('p','',owned('source',()=>({mode:i18n().t('communication.review_context.mode_'+reply.mode.toLowerCase()),id:reply.source_id,revision:i18n().number(reply.source_revision)}))),node('p','',h.available?(h.in_reply_to.length?owned('in_reply_to',{ids:h.in_reply_to.join(', ')}):owned('no_in_reply_to')):owned('headers_unavailable')));
    if(h.references.length)panel.append(node('p','',owned('references',{ids:h.references.join(', ')})));panel.append(node('p','',owned('unverified')));
  }
        ...
        for(const message of model.items){
          ...
          details.addEventListener('toggle',async()=>{
            ...
            try{
              ...
              appendMailboxSourceDownload(record,body,active,view.request_context);appendCommunicationDeliveryReport(record,body,active,view.request_context);if(view.can_view_conversation)appendCommunicationConversation(record,body,active,view.request_context);
              if(view.can_write){
