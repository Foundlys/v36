...
const sandbox={crypto,URL,URLSearchParams,FoundlyI18n:require('./foundly-i18n').create('nl-NL'),document:{createElement:tag=>new Element(tag),addEventListener(){}}};vm.createContext(sandbox);for(const file of ['procurement-clarifications-client.js','procurement-zero-client.js'])vm.runInContext(fs.readFileSync(file,'utf8'),sandbox);let view,active=true;
