'use strict';
(function(root){
 function create({document,workflow,inputHost,getInput,canRequest,onBusy,zeroRequest,isActive=()=>true}){
  ...
  const active=()=>box.isConnected&&inputHost.isConnected&&isActive();
  prepare.addEventListener('click',async()=>{
   ...
   }catch(e){if(active()&&generation===start)notice.textContent=e.message;}finally{busy=false;if(active()){onBusy(false);prepare.disabled=false;}}
  });
  confirm.addEventListener('click',async()=>{
   ...
   }catch(e){if(active()&&preview===p){notice.textContent=e.message;confirm.disabled=false;}}finally{busy=false;if(active()){onBusy(false);prepare.disabled=false;}}
  });return box;
 }
