...
(async()=>{try{
 ...
 await stop();await start();await new Promise(resolve=>setTimeout(resolve,Math.max(0,Date.parse(run.next_wakeup_at)-Date.now()+10)));assert.equal((await call(route,'POST',{event,options})).body.status,'WAITING_TIME');run=(await resume(call,run)).body;assert.equal(run.status,'SUCCEEDED');assert.equal(run.steps[3].status,'SKIPPED_CONDITION');assert.equal((await call('/api/automation/tasks')).body.total,2);assert.equal((await call(route,'POST',{event,options})).body.replayed,true);
 ...
 const reused=await call(route,'POST',{event:falseEvent,options:{...falseOptions,approval:{...approval,run_id:run.run_id,request_signature:run.request_signature}}});assert.equal(reused.status,409);assert.equal(reused.body.code,'automation_approval_request_conflict');assert.equal((await call('/api/automation/tasks')).body.total,2);
 run=(await call(route,'POST',{event:falseEvent,options:{...falseOptions,approval:{...approval,reference:'http-false-branch:review',run_id:run.run_id,request_signature:run.request_signature}}})).body;assert.equal(run.status,'SUCCEEDED');assert.equal((await call('/api/automation/tasks')).body.total,3);
