...
const {signature,validateWorkflow}=require('./workflow-execution');
...
function previewRecovery(core,ctx,actor,runId){
  ...
  const internal=['create_task','create_document'].includes(type),absent=internal&&proof?.verified===false&&proof.absence_verified===true&&proof.outcome==='ABSENT'&&proof.idempotency_key===step.idempotency_key&&typeof proof.record_fingerprint==='string'&&/^[a-f0-9]{64}$/.test(proof.record_fingerprint)&&proof.entity===(type==='create_task'?'tasks':'documents')&&proof.external_write===false;
  const present=proof?.verified===true&&typeof proof.record_id==='string'&&Number.isSafeInteger(proof.record_revision)&&typeof proof.record_fingerprint==='string'&&proof.idempotency_key===step.idempotency_key&&proof.external_write===false;
