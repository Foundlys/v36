...
(async()=>{try{
  ...
  const p=await get('/api/connector-runtime/profiles'),s=await get('/api/connectors');assert.equal(p.r.status,200);assert.equal(s.r.status,200);assert.equal(p.j.total,100);assert.equal(s.j.total,100);
  const profiles=new Map(p.j.profiles.map(x=>[x.id,x])),statuses=new Map(s.j.connectors.map(x=>[x.id,x]));assert.equal(profiles.size,100);assert.equal(statuses.size,100);
  ...
  const html=fs.readFileSync(__dirname+'/index.html','utf8'),js=fs.readFileSync(__dirname+'/index-script.js','utf8'),ui=html+'\n'+js;
  const requiredApi=['/api/zero/turn','/api/integration-sync/','/api/google/connect','/api/connect/${dedicated}','/api/connector-runtime/test/','/api/connector-runtime/config/','/api/events','/api/workers/tick'];for(const token of requiredApi)assert(js.includes(token),`UI missing ${token}`);assert(fs.readFileSync(__dirname+'/server.js','utf8').includes("replace(/^\\/api\\/jarvis(?=\\/|$)/,'/api/zero')"),'legacy Jarvis API prefix must map to ZERO');
  ...
  console.log(JSON.stringify({ok:true,version:'6.0.0',connectors_audited:100,status_contract:'pass',dedicated_routes:'pass',ui_contract:'pass',zero_pipeline:'pass'},null,2));
