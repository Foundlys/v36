...
function metadata(row){const {mime_data,...publicRow}=row;return {...clone(publicRow),delivery_verified:false,retry_available:false,reconciliation_required:uncertain(row),provider_acceptance:row.status==='ACCEPTED_BY_PROVIDER'?true:uncertain(row)?null:false};}
class MailSubmissions{
 ...
 reviewState(ctx,actor,id,review){let canSubmit=false;try{this.sources(ctx,actor,id,review);canSubmit=!blocks(this.core,ctx,id,review.basis_fingerprint);}catch(error){if(![401,403,404,409,422].includes(error.statusCode))throw error;}
  return {can_submit:canSubmit,ready_for_submission:canSubmit,submissions:this.core.adapter.bucket(ctx,SCOPE).filter(row=>row.draft_id===id&&row.review_id===review.id).map(metadata)};
 }
 list(ctx,actor,id){this.read(ctx,actor,id);return {items:this.core.adapter.bucket(ctx,SCOPE).filter(row=>row.draft_id===id).map(metadata),coverage:'RETAINED_SUBMISSION_ATTEMPTS',delivery_verified:false};}
 async execute(ctx,actor,id,reviewId,input,{idempotency_key}={}){
  ...
  try{this.core.mutate(ctx,()=>{
   const row=current(),at=new Date().toISOString();Object.assign(row,{status,revision:row.revision+1,updated_at:at,smtp_code:Number.isInteger(result?.smtp_code)?result.smtp_code:null,receipt_sha256:accepted?result.receipt_sha256:null,error_code:/^(smtp_|mail_|send_|composition_|core_|identity_)/.test(result?.error_code||'')?result.error_code:null});
   if(accepted){const record={id:attemptId,title:plan.snapshot.title,content:plan.snapshot.content,to:plan.snapshot.to,from:plan.from,attachments:plan.snapshot.attachments,owner_id:review.requester_id,owned_entity:'messages',direction:'OUTBOUND',status:'ACCEPTED_BY_PROVIDER',delivery_state:'ACCEPTED_BY_PROVIDER',delivery_verified:false,provider:'email',internet_message_id:message.message_id,provider_message_id:null,submission_id:attemptId,source_draft_id:id,review_id:reviewId,payload_sha256:message.sha256,sent_at:at,created_at:at,updated_at:at,revision:1,schema_version:1,provenance:{source:'SMTP_SUBMISSION',acceptance_verified:true,delivery_verified:false,receipt_sha256:result.receipt_sha256}};this.core.bucket(ctx,'messages').push(record);this.core.recordEvent(ctx,receiptActor,'messages',record,'created');}
