...
test('actual financial, CRM and vehicle presentation preserves calendar dates and does not fabricate zero from missing values',async()=>{
 ...
 vm.runInContext(extract('finance-script.js','money')+extract('finance-script.js','formatDate'),context);assert.equal(context.money(123450,'EUR'),'€1,234.50');assert.equal(context.money(-1,'EUR'),'-€0.01');assert.equal(context.money(123450),'Unknown');assert.equal(context.formatDate('2026-01-01'),'1 Jan 2026');
