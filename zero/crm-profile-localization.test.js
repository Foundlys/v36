...
test('native CRM profile defaults use the explicit locale and preserve canonical stages, customer names and persisted edits on replay',()=>{
 for(const locale of locales){
  ...
  const input={...profile,locale},options={idempotencyKey:'profile-language-'+locale},result=core.provisionProfile(ctx,actor,input,options),expected=defaults(create(locale));
  assert.equal(result.profile.crm_config.locale,locale);assert.equal(result.profile.business_name,profile.business_name);assert.deepEqual(result.stages.map(row=>({name:row.name,probability:row.probability,status:row.status})),expected);
  ...
  assert.throws(()=>core.provisionProfile(ctx,actor,{...input,locale:locale==='nl-NL'?'en-GB':'nl-NL'},options),error=>error.code==='crm_idempotency_conflict');
 ...
 assert.throws(()=>core.provisionProfile(ctx,actor,{...profile,locale:'xx-XX'}),error=>error.code==='crm_profile_locale_invalid');assert.equal(store.size(),0);
...
test('actual CRM profile form snapshots the interface locale and handles a lost committed response without duplicate configuration',async()=>{
 for(const locale of locales){
  ...
  f.i18n.setLocale(locale==='sv-SE'?'de-DE':'sv-SE');f.inputs[0].value='Changed after submission';await f.submit();assert.equal(f.formReads(),1);assert.deepEqual(f.calls[0],f.calls[1]);assert.equal(JSON.parse(f.calls[1].body).locale,locale);assert.equal(f.core.list(f.ctx,f.actor,'pipelines').total,1);assert.equal(f.core.list(f.ctx,f.actor,'stages').total,6);assert.equal(f.state.provisionRequest,null);assert.ok(f.inputs.every(node=>!node.disabled&&node.value===''));assert.ok(f.state.provisionNotice.textContent.includes(profile.business_name));
 }
});
...
test('native CRM HTTP profile locale is validated, persisted and replayed independently of conversation language',async()=>{
 const f=await fixture();try{
  ...
  const payload={...profile,locale:'fr-FR'},headers={'idempotency-key':'profile-locale-http'};let r=await f.request('/api/crm/provision','POST',payload,null,headers);assert.equal(r.status,201);assert.equal(r.body.stages[0].name,'Nouveau');assert.equal(r.body.profile.crm_config.locale,'fr-FR');const pipeline=r.body.pipeline.id;
  ...
  r=await f.request('/api/crm/provision','POST',{...profile,locale:'unsupported'});assert.equal(r.status,400);assert.equal(r.body.code,'crm_profile_locale_invalid');assert.equal((await f.request('/api/zero/preferences')).body.preferences.language,'de-DE');
