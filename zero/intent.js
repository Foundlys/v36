...
const COMMAND=/^(?:(?:please|graag|alstublieft|bitte|s'il vous plaît|por favor|venligst|vær så snill|snälla)\s+)?(?:(?:kun|wil|kan|zou)\s+je\s+|(?:can|could|will|would)\s+you\s+)?(?:sync|synchroniseer|ververs|haal|update|herstel|controleer|test|voeg|registreer|maak|plan|zet|genereer|stel\s+op|schrijf|bereid|create|add|schedule|draft|prepare|refresh|erstelle|plane|crée|créer|prépare|planifie|crea|prepara|programa|opret|planlæg|opprett|lag|planlegg|skapa|planera)\b/iu;
