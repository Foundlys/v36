...
async function setup(){
 ...
 try{assert.equal((await f.request('/api/composition','PUT',{entitlements:['procurement','crm'],expected_revision:0})).status,200);assert.equal((await f.request('/api/connector-runtime/profile/rdw','PUT',{base_url:'https://source-sync.fixture.test',health:{path:'/health',method:'GET'}})).status,200);const member=await f.enroll('probe.authority.manager',['ADMIN']);
  return {...f,file,set,member,probe:()=>f.request('/api/integration-test/rdw','POST',{},member.cookie),fetches:()=>Number(fs.existsSync(file('fetches'))?fs.readFileSync(file('fetches'),'utf8'):0),async waitFor(name){for(let n=0;n<200&&!fs.existsSync(file(name));n++)await new Promise(resolve=>setTimeout(resolve,20));assert.ok(fs.existsSync(file(name)),name);},async revoke(){assert.equal((await f.request('/api/identity/users/'+member.member.id,'PUT',{roles:['VIEWER'],expected_revision:member.member.revision,confirm:true,reason:'Isolated provider verification rights revocation'})).status,200);},async close(){fs.writeFileSync(file('release'),'release');fs.writeFileSync(file('dns-release'),'release');await f.close();fs.rmSync(dir,{recursive:true,force:true});}};
 }catch(error){await f.close();fs.rmSync(dir,{recursive:true,force:true});throw error;}
}
for(const phase of ['DNS','provider']){
 test('legacy generic probe refuses rights revoked during '+phase,async()=>{
 ...
 test('legacy generic probe rejects configuration revision changed during '+phase,async()=>{
...
test('legacy generic probe rejects a changed runtime profile before accepting the old provider response',async()=>{
...
test('legacy generic probe cannot return connector details after the owner module is disabled',async()=>{
...
test('legacy generic probe preserves explicit provider failure and binds a current success to the configuration revision',async()=>{
...
test('legacy generic probe detects a legacy credential change during its provider wait',async()=>{
 const f=await setup();try{f.set({wait:true});const pending=f.probe();await f.waitFor('started');assert.equal((await f.request('/api/integration-config/rdw','POST',{api_key:'isolated replacement legacy key'})).status,200);fs.writeFileSync(f.file('release'),'resume');assert.equal((await pending).status,409);}finally{await f.close();}
});
test('legacy verification only contacts the selected provider',async()=>{
 const f=await setup();try{const r=await f.probe();assert.equal(r.status,200);assert.equal(r.body.connector.connected,true);assert.equal(f.fetches(),1);assert.equal(Number(fs.existsSync(f.file('unrelated-fetches'))?fs.readFileSync(f.file('unrelated-fetches'),'utf8'):0),0);}finally{await f.close();}
});
