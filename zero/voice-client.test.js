...
test('actual client Realtime handlers wait for transcription and dispatch duplicate model calls only once',async()=>{
 ...
 const presentation=source.slice(source.indexOf('function zeroCopy('),source.indexOf('function zeroCopyPart('));
 vm.runInNewContext(presentation+'\n'+source.slice(source.indexOf('function realtimeFunctionCalls('),source.indexOf('function initialCommandState(')),context);
