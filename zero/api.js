...
function createZeroApi({context,principal,body,json,memory,stack,declareStack,models,agents}) {
  return async function handle(req,res,url) {
    const route=url.pathname.replace(/^\/api\/jarvis(?=\/|$)/,'/api/zero');
    if(!/^\/api\/zero\/(?:memories(?:\/|$)|stack(?:\/evidence)?$|models$|agents$|plans(?:\/|$))/.test(route))return false;
    try{
