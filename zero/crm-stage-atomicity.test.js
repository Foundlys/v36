...
test('native CRM stage event identifier collision rolls back the associated deal update',()=>{
 const f=fixture();f.core.evaluateAutomations(f.ctx,f.actor,{...f.event,record_id:'different-source'});const before=f.snapshot(),writes=f.count(),events=f.events.length;assert.throws(()=>f.move(),e=>e.code==='crm_event_replay_conflict');assert.equal(f.snapshot(),before);assert.equal(f.count(),writes);assert.equal(f.events.length,events);
});
