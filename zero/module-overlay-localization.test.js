...
test('Automotive registry preserves source IDs, observes each native lifecycle, and treats missing or duplicate evidence as unknown',async()=>{
  ...
  for(const status of require('../foundly-registry').CONNECTOR_LIFECYCLE){panel.render([{source_id:'rdw',connection_status:status}]);for(const locale of locales){f.i.setLocale(locale);assert.equal(panel.card.children[2].children[0].children[1].children[0].textContent,f.i.t('overlay.source.'+status.toLowerCase()));}}
