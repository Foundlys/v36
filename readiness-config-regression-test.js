...
(async()=>{try{
  ...
  await withServer({
    ...
    const {response,body}=await getReady(base);assert.equal(response.status,503);assert.equal(body.version,'6.0.0');assert.equal(body.checks.authentication,true);assert.equal(body.checks.encryption,true);assert.equal(body.checks.public_base_url,true);assert.equal(body.checks.oauth_callbacks,true);assert.deepEqual(body.failed,['storage_path','persistent_mount']);
  });

  console.log(JSON.stringify({ok:true,version:'6.0.0',configured_admin_secret_accepted:'pass',placeholder_encryption_rejected:'pass',localhost_origin_rejected:'pass',callback_exactness:'pass',derived_callbacks:'pass'},null,2));
